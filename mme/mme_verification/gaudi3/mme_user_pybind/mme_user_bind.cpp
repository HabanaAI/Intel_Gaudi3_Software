...
#include "gaudi3/mme_user/gaudi3_mme_user.h"
...
PYBIND11_MODULE(mme_user_utils_gaudi3, m)
{
    ...
    m.def("gen_mme_cmd_desc",
          [](uint32_t opType,
             ...
             uint64_t smBase,
             void* progs_ptr) {
              const MmeCommon::MmeHalReader& mmeHal = gaudi3::MmeHalReader::getInstance();
              // Create SO manager
              Gaudi3SyncObjectManager soMgr(smBase);

              // Setup all the parameters for createActivations() mme user entrypoint
              TestResources testResources({});

              // Create mmeUser
              Gaudi3MmeUser mmeUser;
              ...
              testResources.numOfSoIdx = gaudi3::Mme::MME_CORE_MASTERS_NR;  // no slave signaling, no secondary output
              testDataParams.syncObjects.resize(gaudi3::Mme::MME_CORE_MASTERS_NR);
              testDataParams.soValues.resize(gaudi3::Mme::MME_CORE_MASTERS_NR);
              ...
              mmeUser.patchTensors(testDataParams,
                                   0,  // sramOffset
                                   0);  // hbmOffset
              mmeUser.buildCmds(testParams, testResources.cmds.data(), testResources.testDataParams, firstValidTest, testResources.testMemUsage);
