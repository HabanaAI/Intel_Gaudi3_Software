...
std::function<void()> wrapFunction(F f, StartParams& startParams, unsigned threadId, uint64_t& time, Args... args)
{
...
measure(F f, const char* name, TestParams params, std::function<void()> initFunc = nullptr)
{
    ...
    for (unsigned cur_thread_count = params.minNbThreads; cur_thread_count <= params.maxNbThreads; ++cur_thread_count)
    {
        ...
        int64_t avrg_full_cpu_execution_time = 0;
        int64_t min_full_cpu_execution_time  = 9999999;
        int64_t max_full_cpu_execution_time  = 0;
        ...
        for (unsigned i = 0; i < params.nbTests; ++i)
        {
            ...
            int64_t full_cpu_execution_time = (t1.tv_sec - t0.tv_sec) * 1000000ll + (t1.tv_nsec - t0.tv_nsec) / 1000;
            ...
            avrg_full_cpu_execution_time += full_cpu_execution_time;
            min_full_cpu_execution_time = std::min(min_full_cpu_execution_time, full_cpu_execution_time);
            max_full_cpu_execution_time = std::max(max_full_cpu_execution_time, full_cpu_execution_time);
            if (initFunc)
            {
                initFunc();
            }
        ...
        avrg_full_cpu_execution_time /= params.nbTests;
        ...
        result.minCpuTimeUs  = min_full_cpu_execution_time / double(params.internalLoopInterations);
        result.avrgCpuTimeUs = avrg_full_cpu_execution_time / double(params.internalLoopInterations);
        result.maxCpuTimeUs  = max_full_cpu_execution_time / double(params.internalLoopInterations);
    ...
    for (auto const& res : results)
    {
        ...
        for (auto nbThreads : threads)
        {
            ...
            for (auto const& opRes : res.operationResults)
            {
                ...
                {
                    sprintf(maxBuf,  "%*.3f/%.3f|", valFieldSize - 1, opRes.maxTimeUs,  opRes.maxCpuTimeUs);
                    sprintf(avrgBuf, "%*.3f/%.3f|", valFieldSize - 1, opRes.avrgTimeUs, opRes.avrgCpuTimeUs);
                    sprintf(minBuf,  "%*.3f/%.3f|", valFieldSize - 1, opRes.minTimeUs,  opRes.minCpuTimeUs);
                    break;
                }
            }
            maxLine += maxBuf;
