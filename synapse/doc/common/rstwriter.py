...
class RstWriter:
    ...
    def title(self, text):
        ...
        if self.is_sphinx_mode:
            ...
            indent = 1
            paragraph = "\n\n".join(
                " " * indent + para.strip() for para in text.strip().split("\n")
            )
            paragraph += "\n"
            heading = ".. class:: title\n\n" + paragraph
    ...
    def paragraph(self, text, indent=0):
        ...
        text = text.split("\n")
        paragraph = "\n\n".join(
            " " * indent + para.strip() for para in text if para.strip()
        )
        paragraph += "\n"
    ...
    def _construct_table(self, num_rows, num_cols, data, header=[], indent=0):
        ...
        if header:
            tdata += ("\n " + " " * indent).join(
                [row_separator, th, header_separator]
            ) + ("\n " + " " * indent)
        else:
            tdata += row_separator + ("\n " + " " * indent)
    ...
    def insert_image(self, path, width="", height="", align=""):
    ...
    def page_break(self, mode=0, template="oneColumn"):
        """
        Break to a new page
        Parameters:
    ...
    def _header_or_footer(self, content):
        ...
        table_str = directive + self._construct_table(
            1, len(content), [content_with_align], indent=1
        )
        return table_str
    ...
    def _header(self, content):
        ...
        COMPANY_ICON_PATH = os.path.join(
            os.environ["SYNAPSE_ROOT"], "doc/common/habana-Icon_BLUE.png"
        )
        self.substitution(
