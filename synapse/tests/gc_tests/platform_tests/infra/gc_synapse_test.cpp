#include "gc_tests/platform_tests/infra/gc_synapse_test.h"
#include "gc_tests/platform_tests/infra/shared_resources.h"
...
: m_isInitialized(false),
  ...
  m_testGlobalConfig(SharedResources::config())
  {
    if (m_testGlobalConfig.deviceType.has_value())
    {
        m_deviceType = m_testGlobalConfig.deviceType.value();
    }
...
void SynTest::SetUp()
{
    if (!shouldRunTest())
    {
        // some tests requires synInitialize on test setup which is called only on AcquireDevices
        if (!TestDeviceManager::instance().synInitialized())
        {
            m_ctx = syn::Context();
        }
        GTEST_SKIP() << m_testConfig.skipReason();
    }
    ...
    SetTestFileName();
    LOG_WARN(SYN_TEST, "Test Name: {}", ::testing::UnitTest::GetInstance()->current_test_info()->name());
    m_setupStatus = true;
}
...
{
    m_deviceIds.clear();
    m_devices.push_back(TestDeviceManager::instance().acquireDevice(m_deviceType));
    m_deviceIds.push_back(m_devices.back()->dev().getInfo().deviceId);
                                                                    ...
                                                                    uint64_t             offset)
{
    synStatus status;
    synTensor tensor;
    status = synTensorHandleCreate(&tensor, graphHandle, DATA_TENSOR, name);
    assert(status == synSuccess && "Create tensor failed!");

    synTensorGeometry geometry = {};
    geometry.dims              = dims;
    std::reverse_copy(tensorSize, tensorSize + dims, geometry.sizes);
    status = synTensorSetGeometry(tensor, &geometry, synGeometryMaxSizes);
    assert(status == synSuccess && "setting tensor max shape failed!");
    status = synTensorSetDeviceDataType(tensor, dataType);
    assert(status == synSuccess && "setting tensor data type failed!");

    synSectionHandle createdSectionHandle = nullptr;
    if (isPersist)
    {
        ...
        if (!sectionHandle)
        {
            ...
            assert(status == synSuccess && "Create tensor failed!");
            createdSectionHandle = sectionHandle;
        }
        status = synTensorAssignToSection(tensor, sectionHandle, offset);
        assert(status == synSuccess && "section assignment to tensor failed!");
        UNUSED(status);
    }
    return {tensor, createdSectionHandle};
}
