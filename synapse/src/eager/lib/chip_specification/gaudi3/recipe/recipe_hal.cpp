...
#include "chip_specification/gaudi3/recipe/command_packets_wrappers.h"
...
{
static_assert(halFullChipSpecificInfo.numMmeEngines <= RecipeHalBase::maxMmeEnginesNr);
...
{
    // execution blob = 536, dynamic blob = 100, patching blob = 32
    constexpr unsigned CONSTANT_BLOBS_SIZE = 668;
    // constant kernel size is identical regardless of type
    constexpr unsigned CONSTANT_KERNEL_SIZE = 3008;
...
StructSizeType RecipeHal::getFirstRegPosInDesc(EngineType engineType, SecondaryEngineType secondaryEngineType) const
{
    return getRegForLoadDesc(engineType2HabanaDeviceType(engineType),
                             secondaryEngineType == SecondaryEngineType::TRANSPOSE);
}
...
unsigned RecipeHal::getMaxEngines(EngineType engineType) const
{
    ...
    {
        case EngineType::TPC:
            return halFullChipSpecificInfo.numTpcEngines;
        case EngineType::MME:
            return halFullChipSpecificInfo.numMmeEngines;
        default:
...
{
    return Gaudi3RecipeGenerator::getGaudi3VersionMinor();
}
...
AsicRange RecipeHal::getMmeDescriptorBlackListRange(SecondaryEngineType secondaryEngineType) const
{
    bool        isTranspose    = (secondaryEngineType == SecondaryEngineType::TRANSPOSE);
    AsicRegType inclusiveFirst = mme_regs::getBlackListInclusiveFirst(isTranspose);
    AsicRegType inclusiveLast  = mme_regs::getBlackListInclusiveLast(isTranspose);
...
{
    return AsicRange {tpc_regs::ICACHE_AXI_CFG, tpc_regs::DCACHE_AXI_CFG};
}

StructSizeType RecipeHal::getDmaTensorInputOffsetInDescriptor() const
{
    EAGER_ASSERT(false, "DMA is not supported for GC in Gaudi3");
    return 0;
}
