...
{
    irt_desc_par irt_desc = {0};
    irt_cfg_pars irt_cfg  = {0};
    rotation_par rot_pars = {0};
                                ...
                                RotatorDesc& desc)
{
    ...
    desc.out_img.width                  = irt_desc.image_par[OIMAGE].W;
    desc.out_stride.val                 = irt_desc.image_par[OIMAGE].Hs;
    desc.out_stripe.width               = irt_desc.image_par[OIMAGE].S;
    ...
    desc.ctrl_cfg.int_mode              = irt_desc.int_mode;
    ...
    HB_ASSERT(irt_cfg.buf_mode[0] <= 7,
              "Buf_mode {}: Rotator engine cannot operate on this configuration",
              irt_cfg.buf_mode[0]);
    HB_ASSERT(irt_cfg.buf_mode[1] <= 7,
              "Buf_mode {}: Rotator engine cannot operate on this configuration",
              irt_cfg.buf_mode[1]);

    desc.buf_cfg.rot_epl           = 1 << irt_cfg.buf_mode[0];
    desc.buf_cfg.mesh_epl          = 1 << irt_cfg.buf_mode[1];
    desc.buf_cfg.mesh_mode         = irt_cfg.buf_mode[1];
    desc.buf_cfg.mesh_fmt          = irt_cfg.buf_format[1];
    desc.in_img_axi_cfg.mcid       = 0;
    desc.in_img_axi_cfg.class_rd   = 0;
    desc.in_img_axi_cfg.arcache    = 7;
    desc.out_img_axi_cfg.mcid      = 0;
    desc.out_img_axi_cfg.class_wr  = 0;
    desc.out_img_axi_cfg.awcache   = 7;
    // set rotator block BW optimization
    desc.mesh_irm_rl._raw          = 0x1ff01;
    desc.grad_irm_rl._raw          = 0x1ff07;
                                                     ...
                                                     RotDescriptorsList&       descriptors)
{
    ...
    uint32_t        outputImageHeight = outputDims[NCHW_H_DIM];
    ...
    SET_MASK_BULK_ON(std::begin(descMask),
                     ...
                     MASK_OFFSET(RotatorDesc, _pad124));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, owm_cfg),
                     MASK_OFFSET(RotatorDesc, _pad268));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, rescale_cfg),
                     MASK_OFFSET(RotatorDesc, _pad284));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, grad_stride),
                     MASK_OFFSET(RotatorDesc, _pad292));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, grad_ctrl),
                     MASK_OFFSET(RotatorDesc, _pad300));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, rot_irm_rl),
                     MASK_OFFSET(RotatorDesc, _pad312));

    SET_MASK_BULK_ON(std::begin(descMask),
                     MASK_OFFSET(RotatorDesc, grad_irm_rl),
                     MASK_OFFSET(RotatorDesc, _pad348));
    ...
    rot_pars.rot_prec_auto_adj           = 1;
    rot_pars.oimg_auto_adj               = 1;
    rot_pars.mesh_mode                   = 1;
    rot_pars.buf_format[0]              = e_irt_buf_format_dynamic;
    rot_pars.buf_select[0]              = e_irt_buf_select_auto;
                                                            ...
                                                            uint64_t                   outImgAddr)
{
    ...
    SizeArray inputDims  = {14, 14, 1, 1};
    SizeArray outputDims = {14, 14, 1, 1};
