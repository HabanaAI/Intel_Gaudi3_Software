...
class small_map {
 ...
 public:
  ...
  data_type& at(const key_type& key) {
    ...
    if (iter == end()) {
        throw std::out_of_range("key not found in small map");
    }
    return iter->second;
  }
    ...
    for(auto current = f; current != l; ++current)
    {
        if (std::none_of(std::begin(array_), 
                        ...
                        [&current, &compare](const auto& element){ 
                          return compare(element.first, current->first);
                        })) {
          if (size_ == kArraySize) {
            ConvertToRealMap();
            map_.insert(current, l);
            return;              
          }
...
template <typename NormalMap, size_t kArraySize, typename EqualKey, typename MapInit>
bool operator==(const small_map<NormalMap, kArraySize, EqualKey, MapInit> &lhs,
                const small_map<NormalMap, kArraySize, EqualKey, MapInit> &rhs) {
  if (lhs.size() != rhs.size()) return false;
  return std::all_of(lhs.begin(),
                     lhs.end(),
                     [&rhs](const typename NormalMap::value_type &element) {
                      auto iter = rhs.find(element.first);
                      return iter != rhs.end() && iter->second == element.second; 
                    });
}
