...
extern float*** m_warp_data; // input
extern float*** m_warp_grad_data; // output
extern float*** input_im_grad_data; // output
extern float*** input_im_data; // input image
extern float*** output_im_data; // output image
...
void IRThl_advanced_transform(IRT_top* irt_top,
                              uint8_t  image,
                              uint8_t  planes,
                              char     file_name[50],
                              uint8_t  transform_type)
{
    ...
    Eirt_mesh_format_type mesh_format = irt_top->irt_desc[desc].mesh_format;
    ...
    double   xo_d = 0, yo_d = 0, xi_fpt64 = 0, yi_fpt64 = 0, xf_fpt64 = 0, yf_fpt64 = 0, out_fpt64 = 0, xim_fpt64 = 0,
           yim_fpt64 = 0;
    ...
    float  xi_fpt32 = 0, yi_fpt32 = 0, xo_f = 0, xo_arch = 0, yo_f = 0, xim_fpt32 = 0, yim_fpt32 = 0, xf_fpt32 = 0,
          yf_fpt32 = 0, xf_arch = 0, yf_arch = 0, out_fpt32 = 0, xi_farch = 0, yi_farch = 0;
    float   xw0, xw1, yw0, yw1;
    int64_t xi_fixed = 0, yi_fixed = 0, xim_fixed = 0, yim_fixed = 0, xi_fix31 = 0, yi_fix31 = 0, xim_arch = 0,
            yim_arch  = 0;
    ...
    bool     oob_flag_h[CALC_FORMATS] = {0}, oob_flag_v[CALC_FORMATS] = {0}, noint_flag_h[CALC_FORMATS] = {0},
         noint_flag_v[CALC_FORMATS] = {0};

    int16_t             row_bank_access[IRT_ROT_MEM_ROW_BANKS][5] = {0};
    irt_rot_mem_rd_stat rot_mem_rd_stat;

    // int16_t Yi_start = (int16_t)(irt_top->irt_desc[desc].Yi_first_fixed >> IRT_SLOPE_PREC) -
    // irt_top->irt_desc[desc].read_vflip; uint16_t BufW_entries =
    // irt_top->irt_cfg.rm_cfg[irt_rot][irt_top->irt_desc[desc].image_par[IIMAGE].buf_mode].Buf_EpL;// <<
    // irt_top->irt_desc[irt_task].image_par[IIMAGE].Ps; uint8_t Ps1 = irt_top->irt_cfg.buf_1b_mode[irt_rot] == 1 ||
    // irt_top->irt_desc[desc].image_par[IIMAGE].Ps == 1;
    ...
    IRT_TRACE_UTILS("\nRunning %s high level model on image %d %s %dx%d with ",
    ...
    if (irt_top->irt_desc[desc].irt_mode == e_irt_projection && irt_top->rot_pars[desc].proj_mode == e_irt_projection ||
        ...
        IRT_TRACE_UTILS("%s order, roll %3.2f, pitch %3.2f, yaw %3.2f, Sx %3.2f, Sy %3.2f, Zd %3.2f, Wd %3.2f %s shear "
                        "mode, X shear %3.2f, Y shear %3.2f\n",
                        ...
                        irt_top->rot_pars[desc].Sx,
                        irt_top->rot_pars[desc].Sy,
                        irt_top->rot_pars[desc].proj_Zd,
                        irt_top->rot_pars[desc].proj_Wd,
                        irt_shr_mode_s[irt_top->rot_pars[desc].shear_mode],
    ...
    if (irt_top->irt_desc[desc].irt_mode == e_irt_mesh && irt_top->rot_pars[desc].mesh_mode == e_irt_mesh)
        IRT_TRACE_UTILS("%3.2f distortion\n", irt_top->rot_pars[desc].dist_r);
    ...
    for (uint16_t row = 0; row < Ho; row++) {
        for (uint16_t col = 0; col < So; col += adj_proc_size) {
            ...
            for (int i = 0; i < IRT_ROT_MEM_ROW_BANKS; i++) {
                row_bank_access[i][0] = 0;
                row_bank_access[i][1] = 10000;
                row_bank_access[i][2] = 0;
                row_bank_access[i][3] = 10000;
                row_bank_access[i][4] = 0;
            }
            ...
            for (uint8_t pixel = 0; pixel < adj_proc_size; pixel++) {
                ...
                switch (transform_type) {
                    case e_irt_rotation:
                        xi_fpt64 =
                            irt_top->rot_pars[desc].cosd * xo_d + irt_top->rot_pars[desc].sind * yo_d + (double)Xic / 2;
                        yi_fpt64 = -irt_top->rot_pars[desc].sind * xo_d + irt_top->rot_pars[desc].cosd * yo_d +
                                   (double)Yic / 2;
                        xi_fpt32 =
                            irt_top->irt_desc[desc].cosf * xo_f + irt_top->irt_desc[desc].sinf * yo_f + (float)Xic / 2;
                        yi_fpt32 =
                            -irt_top->irt_desc[desc].sinf * xo_f + irt_top->irt_desc[desc].cosf * yo_f + (float)Yic / 2;
                        xi_fixed = (int64_t)irt_top->irt_desc[desc].cosi * xo_i +
                                   (int64_t)irt_top->irt_desc[desc].sini * yo_i +
                                   ((int64_t)Xic << irt_top->rot_pars[desc].TOTAL_PREC);
                        yi_fixed = -(int64_t)irt_top->irt_desc[desc].sini * xo_i +
                                   (int64_t)irt_top->irt_desc[desc].cosi * yo_i +
                                   ((int64_t)Yic << irt_top->rot_pars[desc].TOTAL_PREC);
                        xi_fix16 = irt_top->rot_pars[desc].cos16 * xo_i + irt_top->rot_pars[desc].sin16 * yo_i +
                                   ((int32_t)Xic << 16);
                        yi_fix16 = -irt_top->rot_pars[desc].sin16 * xo_i + irt_top->rot_pars[desc].cos16 * yo_i +
                                   ((int32_t)Yic << 16);
                        // xi_farch = ( irt_top->irt_desc[desc].cosf * (xo_f - pixel) + irt_top->irt_desc[desc].sinf *
                        // yo_f) + pixel * irt_top->irt_desc[desc].cosf; yi_farch = (-irt_top->irt_desc[desc].sinf *
                        // (xo_f - pixel) + irt_top->irt_desc[desc].cosf * yo_f) - pixel * irt_top->irt_desc[desc].sinf;
                        xi_farch = (irt_top->irt_desc[desc].cosf * xo_arch + irt_top->irt_desc[desc].sinf * yo_f) +
                                   (xo_f - xo_arch) * irt_top->irt_desc[desc].cosf;
                        yi_farch = (-irt_top->irt_desc[desc].sinf * xo_arch + irt_top->irt_desc[desc].cosf * yo_f) -
                                   (xo_f - xo_arch) * irt_top->irt_desc[desc].sinf;
                        xi_fix31 = (((int64_t)irt_top->irt_desc[desc].cosi * xo_i +
                                     (int64_t)irt_top->irt_desc[desc].sini * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
                        yi_fix31 = ((-(int64_t)irt_top->irt_desc[desc].sini * xo_i +
                                     (int64_t)irt_top->irt_desc[desc].cosi * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
#ifdef IRT_HL_NO_FLIP
                        xi_fpt64 = irt_top->rot_pars[desc].cosd_hl * xo_d + irt_top->rot_pars[desc].sind_hl * yo_d +
                                   (double)Xic / 2;
                        yi_fpt64 = -irt_top->rot_pars[desc].sind_hl * xo_d + irt_top->rot_pars[desc].cosd_hl * yo_d +
                                   (double)Yic / 2;
                        xi_fpt32 = irt_top->rot_pars[desc].cosf_hl * xo_f + irt_top->rot_pars[desc].sinf_hl * yo_f +
                                   (float)Xic / 2;
                        yi_fpt32 = -irt_top->rot_pars[desc].sinf_hl * xo_f + irt_top->rot_pars[desc].cosf_hl * yo_f +
                                   (float)Yic / 2;
                        xi_fixed = (int64_t)irt_top->rot_pars[desc].cosi_hl * xo_i +
                                   (int64_t)irt_top->rot_pars[desc].sini_hl * yo_i +
                                   ((int64_t)Xic << irt_top->rot_pars[desc].TOTAL_PREC);
                        yi_fixed = -(int64_t)irt_top->rot_pars[desc].sini_hl * xo_i +
                                   (int64_t)irt_top->rot_pars[desc].cosi_hl * yo_i +
                                   ((int64_t)Yic << irt_top->rot_pars[desc].TOTAL_PREC);
                        xi_farch =
                            (irt_top->rot_pars[desc].cosf_hl * xo_arch + irt_top->rot_pars[desc].sinf_hl * yo_f) +
                            (xo_f - xo_arch) * irt_top->rot_pars[desc].cosf_hl;
                        yi_farch =
                            (-irt_top->rot_pars[desc].sinf_hl * xo_arch + irt_top->rot_pars[desc].cosf_hl * yo_f) -
                            (xo_f - xo_arch) * irt_top->rot_pars[desc].sinf_hl;
                        xi_fix31 = (((int64_t)irt_top->rot_pars[desc].cosi_hl * xo_i +
                                     (int64_t)irt_top->rot_pars[desc].sini_hl * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
                        yi_fix31 = ((-(int64_t)irt_top->rot_pars[desc].sini_hl * xo_i +
                                     (int64_t)irt_top->rot_pars[desc].cosi_hl * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
#endif
                        break;
                    case e_irt_affine: // affine transform
                        xi_fpt64 =
                            irt_top->rot_pars[desc].M11d * xo_d + irt_top->rot_pars[desc].M12d * yo_d + (double)Xic / 2;
                        yi_fpt64 =
                            irt_top->rot_pars[desc].M21d * xo_d + irt_top->rot_pars[desc].M22d * yo_d + (double)Yic / 2;
                        xi_fpt32 =
                            irt_top->irt_desc[desc].M11f * xo_f + irt_top->irt_desc[desc].M12f * yo_f + (float)Xic / 2;
                        yi_fpt32 =
                            irt_top->irt_desc[desc].M21f * xo_f + irt_top->irt_desc[desc].M22f * yo_f + (float)Yic / 2;
                        xi_fixed = (int64_t)irt_top->irt_desc[desc].M11i * xo_i +
                                   (int64_t)irt_top->irt_desc[desc].M12i * yo_i +
                                   ((int64_t)Xic << irt_top->rot_pars[desc].TOTAL_PREC);
                        yi_fixed = (int64_t)irt_top->irt_desc[desc].M21i * xo_i +
                                   (int64_t)irt_top->irt_desc[desc].M22i * yo_i +
                                   ((int64_t)Yic << irt_top->rot_pars[desc].TOTAL_PREC);
                        // xi_farch = (irt_top->irt_desc[desc].M11f * (xo_f - pixel) + irt_top->irt_desc[desc].M12f *
                        // yo_f) + pixel * irt_top->irt_desc[desc].M11f; yi_farch = (irt_top->irt_desc[desc].M21f *
                        // (xo_f
                        // - pixel) + irt_top->irt_desc[desc].M22f * yo_f) + pixel * irt_top->irt_desc[desc].M21f;
                        xi_farch = (irt_top->irt_desc[desc].M11f * xo_arch + irt_top->irt_desc[desc].M12f * yo_f) +
                                   (xo_f - xo_arch) * irt_top->irt_desc[desc].M11f;
                        yi_farch = (irt_top->irt_desc[desc].M21f * xo_arch + irt_top->irt_desc[desc].M22f * yo_f) +
                                   (xo_f - xo_arch) * irt_top->irt_desc[desc].M21f;
                        xi_fix31 = (((int64_t)irt_top->irt_desc[desc].M11i * xo_i +
                                     (int64_t)irt_top->irt_desc[desc].M12i * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
                        yi_fix31 = (((int64_t)irt_top->irt_desc[desc].M21i * xo_i +
                                     (int64_t)irt_top->irt_desc[desc].M22i * yo_i)
                                    << irt_top->irt_desc[desc].prec_align);
                        break;
                    case e_irt_projection: // projection transform
                        xi_fpt64 = (irt_top->rot_pars[desc].prj_Ad[0] * xo_d +
                                    irt_top->rot_pars[desc].prj_Ad[1] * yo_d + irt_top->rot_pars[desc].prj_Ad[2]) /
                                   (irt_top->rot_pars[desc].prj_Cd[0] * xo_d +
                                    irt_top->rot_pars[desc].prj_Cd[1] * yo_d + irt_top->rot_pars[desc].prj_Cd[2]);
                        yi_fpt64 = (irt_top->rot_pars[desc].prj_Bd[0] * xo_d +
                                    irt_top->rot_pars[desc].prj_Bd[1] * yo_d + irt_top->rot_pars[desc].prj_Bd[2]) /
                                   (irt_top->rot_pars[desc].prj_Dd[0] * xo_d +
                                    irt_top->rot_pars[desc].prj_Dd[1] * yo_d + irt_top->rot_pars[desc].prj_Dd[2]);
                        xi_fpt32 = (irt_top->irt_desc[desc].prj_Af[0] * xo_f +
                                    irt_top->irt_desc[desc].prj_Af[1] * yo_f + irt_top->irt_desc[desc].prj_Af[2]) /
                                   (irt_top->irt_desc[desc].prj_Cf[0] * xo_f +
                                    irt_top->irt_desc[desc].prj_Cf[1] * yo_f + irt_top->irt_desc[desc].prj_Cf[2]);
                        yi_fpt32 = (irt_top->irt_desc[desc].prj_Bf[0] * xo_f +
                                    irt_top->irt_desc[desc].prj_Bf[1] * yo_f + irt_top->irt_desc[desc].prj_Bf[2]) /
                                   (irt_top->irt_desc[desc].prj_Df[0] * xo_f +
                                    irt_top->irt_desc[desc].prj_Df[1] * yo_f + irt_top->irt_desc[desc].prj_Df[2]);
#if 0
					...
					proj_coord_err_hist[proj_slope_S2I][1][std::min((int)floor(fabs(yi_fpt32_inc - yi_fpt64)* PROJ_ERR_BINS + 0.5), PROJ_ERR_BINS - 1)] ++;
                        ...
                        // arch implementation
                        xis_fpt64 =
                            ((irt_top->rot_pars[desc].prj_Ad[0] * xo_arch + irt_top->rot_pars[desc].prj_Ad[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Ad[2]) /
                            ((irt_top->rot_pars[desc].prj_Cd[0] * xo_arch + irt_top->rot_pars[desc].prj_Cd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Cd[2]);
                        xie_fpt64 =
                            ((irt_top->rot_pars[desc].prj_Ad[0] * xo_arch + irt_top->rot_pars[desc].prj_Ad[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Ad[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->rot_pars[desc].prj_Ad[2]) /
                            ((irt_top->rot_pars[desc].prj_Cd[0] * xo_arch + irt_top->rot_pars[desc].prj_Cd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Cd[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->rot_pars[desc].prj_Cd[2]);

                        yis_fpt64 =
                            ((irt_top->rot_pars[desc].prj_Bd[0] * xo_arch + irt_top->rot_pars[desc].prj_Bd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Bd[2]) /
                            ((irt_top->rot_pars[desc].prj_Dd[0] * xo_arch + irt_top->rot_pars[desc].prj_Dd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Dd[2]);
                        yie_fpt64 =
                            ((irt_top->rot_pars[desc].prj_Bd[0] * xo_arch + irt_top->rot_pars[desc].prj_Bd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Bd[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->rot_pars[desc].prj_Bd[2]) /
                            ((irt_top->rot_pars[desc].prj_Dd[0] * xo_arch + irt_top->rot_pars[desc].prj_Dd[1] * yo_d) +
                             irt_top->rot_pars[desc].prj_Dd[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->rot_pars[desc].prj_Dd[2]);
                        ...
                        xis_fpt32 =
                            ((irt_top->irt_desc[desc].prj_Af[0] * xo_arch + irt_top->irt_desc[desc].prj_Af[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Af[2]) /
                            ((irt_top->irt_desc[desc].prj_Cf[0] * xo_arch + irt_top->irt_desc[desc].prj_Cf[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Cf[2]);
                        xie_fpt32 =
                            ((irt_top->irt_desc[desc].prj_Af[0] * xo_arch + irt_top->irt_desc[desc].prj_Af[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Af[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->irt_desc[desc].prj_Af[2]) /
                            ((irt_top->irt_desc[desc].prj_Cf[0] * xo_arch + irt_top->irt_desc[desc].prj_Cf[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Cf[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->irt_desc[desc].prj_Cf[2]);

                        yis_fpt32 =
                            ((irt_top->irt_desc[desc].prj_Bf[0] * xo_arch + irt_top->irt_desc[desc].prj_Bf[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Bf[2]) /
                            ((irt_top->irt_desc[desc].prj_Df[0] * xo_arch + irt_top->irt_desc[desc].prj_Df[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Df[2]);
                        yie_fpt32 =
                            ((irt_top->irt_desc[desc].prj_Bf[0] * xo_arch + irt_top->irt_desc[desc].prj_Bf[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Bf[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->irt_desc[desc].prj_Bf[2]) /
                            ((irt_top->irt_desc[desc].prj_Df[0] * xo_arch + irt_top->irt_desc[desc].prj_Df[1] * yo_f) +
                             irt_top->irt_desc[desc].prj_Df[0] * (irt_top->irt_desc[desc].proc_size - 1) +
                             irt_top->irt_desc[desc].prj_Df[2]);
                    ...
                    case e_irt_mesh: // distortion
                        ...
                        if (irt_top->irt_desc[desc].mesh_sparse_v == 1) { // sparse in v direction
                            ...
                            yim_fpt32 =
                                yim_fpt32 * (float)irt_top->irt_desc[desc].mesh_Gv / pow((float)2, IRT_MESH_G_PREC);
                            yim_fixed =
                                (yim_fixed * (int64_t)irt_top->irt_desc[desc].mesh_Gv + (1 << (IRT_MESH_G_PREC - 1))) >>
                                IRT_MESH_G_PREC;
                        ...
                        if (irt_top->irt_desc[desc].mesh_sparse_h == 1) { // sparse in h direction
                            ...
                            xim_fpt32 =
                                xim_fpt32 * (float)irt_top->irt_desc[desc].mesh_Gh / pow((float)2, IRT_MESH_G_PREC);
                            xim_fixed =
                                (xim_fixed * (int64_t)irt_top->irt_desc[desc].mesh_Gh + (1 << (IRT_MESH_G_PREC - 1))) >>
                                IRT_MESH_G_PREC;
                        ...
                        xf_arch = (float)((double)((xim_arch - ((int64_t)j0[e_irt_mesh_arch] << IRT_MESH_G_PREC)) /
                                                   pow(2.0, IRT_MESH_G_PREC)));
                        ...
                        yf_arch = (float)((double)((yim_arch - ((int64_t)i0[e_irt_mesh_arch] << IRT_MESH_G_PREC)) /
                                                   pow(2.0, IRT_MESH_G_PREC)));
                        ...
                        for (uint8_t format = e_irt_mesh_fpt64; format <= e_irt_mesh_arch; format++) {
                            ...
                            mp_value[0][0] =
                                (float)(p_valid[0] ? ((mesh_format == e_irt_mesh_fp32)
                                                          ? irt_top->irt_cfg.mesh_images
                                                                .mesh_image_fp32[i0[format]][j0[format]]
                                                                .x
                                                          : (float)irt_top->irt_cfg.mesh_images
                                                                    .mesh_image_fi16[i0[format]][j0[format]]
                                                                    .x /
                                                                pow(2.0, irt_top->irt_desc[desc].mesh_point_location))
                                                   : 0);
                            mp_value[0][1] =
                                (float)(p_valid[1] ? ((mesh_format == e_irt_mesh_fp32)
                                                          ? irt_top->irt_cfg.mesh_images
                                                                .mesh_image_fp32[i0[format]][j1[format]]
                                                                .x
                                                          : (float)irt_top->irt_cfg.mesh_images
                                                                    .mesh_image_fi16[i0[format]][j1[format]]
                                                                    .x /
                                                                pow(2.0, irt_top->irt_desc[desc].mesh_point_location))
                                                   : 0);
                            mp_value[0][2] =
                                (float)(p_valid[2] ? ((mesh_format == e_irt_mesh_fp32)
                                                          ? irt_top->irt_cfg.mesh_images
                                                                .mesh_image_fp32[i1[format]][j0[format]]
                                                                .x
                                                          : (float)irt_top->irt_cfg.mesh_images
                                                                    .mesh_image_fi16[i1[format]][j0[format]]
                                                                    .x /
                                                                pow(2.0, irt_top->irt_desc[desc].mesh_point_location))
                                                   : 0);
                            mp_value[0][3] =
                                (float)(p_valid[3] ? ((mesh_format == e_irt_mesh_fp32)
                                                          ? irt_top->irt_cfg.mesh_images
                                                                .mesh_image_fp32[i1[format]][j1[format]]
                                                                .x
                                                          : (float)irt_top->irt_cfg.mesh_images
                                                                    .mesh_image_fi16[i1[format]][j1[format]]
                                                                    .x /
                                                                pow(2.0, irt_top->irt_desc[desc].mesh_point_location))
                                                   : 0);
                                    ...
                                    for (int16_t j = j0[format]; j <= j1[format]; j++) {
                                        ...
                                        for (uint8_t crd = 0; crd <= 1; crd++) {
                                            ...
                                            for (uint16_t byte = 0; byte < mesh_BPC; byte++) {
                                                mp_value_fp32[crd][idx] |=
                                                    ((uint32_t)ext_mem[mimage_crd_start + (uint64_t)byte]
                                                     << (8 * byte));
                                                mp_value_int16[crd][idx] |=
                                                    ((uint16_t)ext_mem[mimage_crd_start + (uint64_t)byte]
                                                     << (8 * byte));
                                            }
                                            mp_value[crd][idx] =
                                                (float)(p_valid[idx] ? ((mesh_format == e_irt_mesh_fp32)
                                                                            ? IRT_top::IRT_UTILS::irt_fp32_to_float(
                                                                                  mp_value_fp32[crd][idx])
                                                                            : (float)mp_value_int16[crd][idx] /
                                                                                  pow(2.0,
                                                                                      irt_top->irt_desc[desc]
                                                                                          .mesh_point_location))
                                                                     : 0);
                                        }
                                        idx++;
                            ...
                            if (format == e_irt_mesh_fpt64) {
                                // printf("[%d, %d] = [%f, %f, %f, %f]\n", row, col + pixel, mp_value[0][0],
                                // mp_value[0][1], mp_value[0][2], mp_value[0][3]); printf("[%d, %d] = [%f, %f, %f,
                                // %f]\n", row, col + pixel, mp_value[1][0], mp_value[1][1], mp_value[1][2],
                                // mp_value[1][3]);
                            }
                        ...
                        for (uint8_t j = 0; j < 2; j++) {
                            ...
                            mi_fpt64[j] =
                                (mp_fpt64[j][0] * (1 - xf_fpt64) + mp_fpt64[j][1] * xf_fpt64) * (1 - yf_fpt64) +
                                (mp_fpt64[j][2] * (1 - xf_fpt64) + mp_fpt64[j][3] * xf_fpt64) * yf_fpt64;
                            mi_fpt32[j] =
                                (mp_fpt32[j][0] * (1 - xf_fpt32) + mp_fpt32[j][1] * xf_fpt32) * (1 - yf_fpt32) +
                                (mp_fpt32[j][2] * (1 - xf_fpt32) + mp_fpt32[j][3] * xf_fpt32) * yf_fpt32;
                            // xi_fixed = mp0_fixed * (1 - xf_fixed) * (1 - yf_fixed) + mp1_fixed * (xf_fixed) * (1 -
                            // yf_fixed) + mp2_fixed * (1 - xf_fixed) * (yf_fixed) + mp3_fixed * (xf_fixed) *
                            // (yf_fixed);
                            mi_fixed[j] =
                                (mp_fixed[j][0] * (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)xf_fixed) +
                                 mp_fixed[j][1] * (double)xf_fixed) *
                                    (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)yf_fixed) +
                                (mp_fixed[j][2] * (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)xf_fixed) +
                                 mp_fixed[j][3] * (double)xf_fixed) *
                                    (double)yf_fixed;
                            ...
                            mi_fpt64[j] =
                                (mp_fpt64[j][0] * (1 - yf_fpt64) + mp_fpt64[j][2] * yf_fpt64) * (1 - xf_fpt64) +
                                (mp_fpt64[j][1] * (1 - yf_fpt64) + mp_fpt64[j][3] * yf_fpt64) * xf_fpt64;
                            mi_fpt32[j] =
                                (mp_fpt32[j][0] * (1 - yf_fpt32) + mp_fpt32[j][2] * yf_fpt32) * (1 - xf_fpt32) +
                                (mp_fpt32[j][1] * (1 - yf_fpt32) + mp_fpt32[j][3] * yf_fpt32) * xf_fpt32;
                            // xi_fixed = mp0_fixed * (1 - xf_fixed) * (1 - yf_fixed) + mp1_fixed * (xf_fixed) * (1 -
                            // yf_fixed) + mp2_fixed * (1 - xf_fixed) * (yf_fixed) + mp3_fixed * (xf_fixed) *
                            // (yf_fixed);
                            mi_fixed[j] =
                                (mp_fixed[j][0] * (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)yf_fixed) +
                                 mp_fixed[j][2] * (double)yf_fixed) *
                                    (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)xf_fixed) +
                                (mp_fixed[j][1] * (pow(2, irt_top->rot_pars[desc].TOTAL_PREC) - (double)yf_fixed) +
                                 mp_fixed[j][3] * (double)yf_fixed) *
                                    (double)xf_fixed;
                            mi_fixed[j] *= pow(4.0, -irt_top->rot_pars[desc].TOTAL_PREC);
                            ...
                            // as in arch model
                            mi_arch[j] = (mp_arch[j][0] * yw1 + mp_arch[j][2] * yw0) * xw1 +
                                         (mp_arch[j][1] * yw1 + mp_arch[j][3] * yw0) * xw0;
                        }
					...
					IRT_TRACE("weights[%8x,%8x] bli input: y: pix[%.8f,%.8f][%.8f,%.8f], x: pix[%.2f,%.2f][%.2f,%.2f], output[X, Y]:[%.8f,%.8f]\n",
                        ...
                        // pixel, yi_fpt32, mp_fpt32_y[0], mp_fpt32_y[1], mp_fpt32_y[2], mp_fpt32_y[3]);
                        // printf("Expended mesh values for X[%d][%d] are [%f] from [%f][%f][%f][%f]\n", row, col +
                        // pixel, xi_fpt32, mp_fpt32_x[0], mp_fpt32_x[1], mp_fpt32_x[2], mp_fpt32_x[3]);
                        break;
                }

                // IRT_TRACE_DBG("[%u, %u]: %f %f\n", row, col + pixel, xi_farch, yi_farch);

                bool use_fixed = irt_top->irt_desc[desc].crd_mode == e_irt_crd_mode_fixed &&
                                 (irt_top->irt_desc[desc].irt_mode == e_irt_rotation ||
                                  irt_top->irt_desc[desc].irt_mode == e_irt_affine);
                bool add_center = irt_top->irt_desc[desc].irt_mode == e_irt_rotation ||
                                  irt_top->irt_desc[desc].irt_mode == e_irt_affine;
                ...
                xi_fix31 = (int64_t)(use_fixed ? xi_fix31 : rint(xi_farch * pow(2, 31))) +
                           (add_center ? ((int64_t)Xic << 30) : 0) + (add_rel ? (((int64_t)col + pixel) << 31) : 0);
                yi_fix31 = (int64_t)(use_fixed ? yi_fix31 : rint(yi_farch * pow(2, 31))) +
                           (add_center ? ((int64_t)Yic << 30) : 0) + (add_rel ? ((int64_t)row << 31) : 0);

                // IRT_TRACE_DBG("[%u, %u]: %f %f\n", row, col + pixel, (double)xi_fix31 / pow(2, 31), (double)yi_fix31
                // / pow(2, 31)); IRT_TRACE_DBG("[%u, %u]: %f %f\n", row, col + pixel, xi_farch, yi_farch);
                ...
                if (irt_top->irt_desc[desc].bg_mode == e_irt_bg_frame_repeat) {
                    for (uint8_t format = 0; format <= e_irt_crdf_fix16; format++) {
                        ...
                        switch (format) {
                            ...
                            case e_irt_crdf_fixed:
                                oob_flag_v[format] = yi_fixed < 0 || yi_fixed > (((int64_t)Hi - 1)
                                                                                 << irt_top->rot_pars[desc].TOTAL_PREC);
                                oob_flag_h[format] = xi_fixed < 0 || xi_fixed > (((int64_t)Wi - 1)
                                                                                 << irt_top->rot_pars[desc].TOTAL_PREC);
                                break;
...
#if 1
                    xi_fix31 = IRT_top::IRT_UTILS::irt_min_int64(IRT_top::IRT_UTILS::irt_max_int64(0, xi_fix31),
                                                                 ((int64_t)Wi - 1) << 31);
                    // yi_fix31 = IRT_top::IRT_UTILS::irt_min_int64(IRT_top::IRT_UTILS::irt_max_int64(0, yi_fix31),
                    // ((int64_t)Hi - 1) << 31);
                    ...
                    xi_fixed =
                        IRT_top::IRT_UTILS::irt_min_int64(IRT_top::IRT_UTILS::irt_max_int64(0, xi_fixed),
                                                          ((int64_t)Wi - 1) << irt_top->rot_pars[desc].TOTAL_PREC);
                    // yi_fixed = IRT_top::IRT_UTILS::irt_min_int64(IRT_top::IRT_UTILS::irt_max_int64(0, yi_fixed),
                    // ((int64_t)Hi - 1) << irt_top->rot_pars[desc].TOTAL_PREC);
                ...
                j1[e_irt_crdf_arch] =
                    j0[e_irt_crdf_arch] + 1 - (oob_flag_h[e_irt_crdf_arch] || noint_flag_h[e_irt_crdf_arch]);
                ...
                i1[e_irt_crdf_arch]           = i0[e_irt_crdf_arch] + 1 -
                                      (irt_top->irt_desc[desc].rot90 && irt_top->irt_desc[desc].rot90_intv == 0 ||
                                       oob_flag_v[e_irt_crdf_arch] || noint_flag_v[e_irt_crdf_arch]);
                ...
                j1[e_irt_crdf_fpt64] =
                    j0[e_irt_crdf_fpt64] + 1 - (oob_flag_h[e_irt_crdf_fpt64] || noint_flag_h[e_irt_crdf_fpt64]);
                ...
                i1[e_irt_crdf_fpt64]           = i0[e_irt_crdf_fpt64] + 1 -
                                       (irt_top->irt_desc[desc].rot90 && irt_top->irt_desc[desc].rot90_intv == 0 ||
                                        oob_flag_v[e_irt_crdf_fpt64] || noint_flag_v[e_irt_crdf_fpt64]);
                ...
                j1[e_irt_crdf_fpt32] =
                    j0[e_irt_crdf_fpt32] + 1 - (oob_flag_h[e_irt_crdf_fpt32] || noint_flag_h[e_irt_crdf_fpt32]);
                ...
                i1[e_irt_crdf_fpt32]           = i0[e_irt_crdf_fpt32] + 1 -
                                       (irt_top->irt_desc[desc].rot90 && irt_top->irt_desc[desc].rot90_intv == 0 ||
                                        oob_flag_v[e_irt_crdf_fpt32] || noint_flag_v[e_irt_crdf_fpt32]);
                ...
                j1[e_irt_crdf_fixed] =
                    j0[e_irt_crdf_fixed] + 1 - (oob_flag_h[e_irt_crdf_fixed] || noint_flag_h[e_irt_crdf_fixed]);
                ...
                i1[e_irt_crdf_fixed]           = i0[e_irt_crdf_fixed] + 1 -
                                       (irt_top->irt_desc[desc].rot90 && irt_top->irt_desc[desc].rot90_intv == 0 ||
                                        oob_flag_v[e_irt_crdf_fixed] || noint_flag_v[e_irt_crdf_fixed]);
                ...
                j1[e_irt_crdf_fix16] =
                    j0[e_irt_crdf_fix16] + 1 - (oob_flag_h[e_irt_crdf_fix16] || noint_flag_h[e_irt_crdf_fix16]);
                ...
                i1[e_irt_crdf_fix16]           = i0[e_irt_crdf_fix16] + 1 -
                                       (irt_top->irt_desc[desc].rot90 && irt_top->irt_desc[desc].rot90_intv == 0 ||
                                        oob_flag_v[e_irt_crdf_fix16] || noint_flag_v[e_irt_crdf_fix16]);
                ...
                uint8_t i0_bank =
                    ((i0[e_irt_crdf_arch] % 8) + 8) %
                    8; // i0_fixed can be negative, additional +8 and %8 required to get %8 value to be positive
                uint8_t i1_bank = ((i1[e_irt_crdf_arch] % 8) + 8) % 8;
                row_bank_access[i0_bank][0]++;
                row_bank_access[i0_bank][1] =
                    IRT_top::IRT_UTILS::irt_min_int16(row_bank_access[i0_bank][1], j0[e_irt_crdf_fixed]);
                row_bank_access[i0_bank][2] =
                    IRT_top::IRT_UTILS::irt_max_int16(row_bank_access[i0_bank][2], j1[e_irt_crdf_fixed]);
                row_bank_access[i0_bank][3] =
                    IRT_top::IRT_UTILS::irt_min_int16(row_bank_access[i0_bank][3], j0[e_irt_crdf_fixed] >> 3);
                row_bank_access[i0_bank][4] =
                    IRT_top::IRT_UTILS::irt_max_int16(row_bank_access[i0_bank][4], j1[e_irt_crdf_fixed] >> 3);

                row_bank_access[i1_bank][0]++;
                row_bank_access[i1_bank][1] =
                    IRT_top::IRT_UTILS::irt_min_int16(row_bank_access[i1_bank][1], j0[e_irt_crdf_fixed]);
                row_bank_access[i1_bank][2] =
                    IRT_top::IRT_UTILS::irt_max_int16(row_bank_access[i1_bank][2], j1[e_irt_crdf_fixed]);
                row_bank_access[i1_bank][3] =
                    IRT_top::IRT_UTILS::irt_min_int16(row_bank_access[i1_bank][3], j0[e_irt_crdf_fixed] >> 3);
                row_bank_access[i1_bank][4] =
                    IRT_top::IRT_UTILS::irt_max_int16(row_bank_access[i1_bank][4], j1[e_irt_crdf_fixed] >> 3);

                irt_cfifo_data_struct rot_mem_rd_ctrl, rot_mem_rd_ctrl1;

                rot_mem_rd_ctrl.YT                                  = i0[e_irt_crdf_arch];
                rot_mem_rd_ctrl.YB                                  = i1[e_irt_crdf_arch];
                rot_mem_rd_ctrl.XL                                  = j0[e_irt_crdf_arch];
                rot_mem_rd_ctrl.XR                                  = j1[e_irt_crdf_arch];
                rot_mem_rd_ctrl.pixel                               = col;
                rot_mem_rd_ctrl.line                                = row;
                irt_top->mem_ctrl[e_irt_block_rot].start_line[desc] = 0;

                rot_mem_rd_ctrl1.YT    = i0[e_irt_crdf_arch];
                rot_mem_rd_ctrl1.YB    = i1[e_irt_crdf_arch];
                rot_mem_rd_ctrl1.XL    = j0[e_irt_crdf_arch];
                rot_mem_rd_ctrl1.XR    = j1[e_irt_crdf_arch];
                rot_mem_rd_ctrl1.pixel = col;
                rot_mem_rd_ctrl1.line  = row;

                bool rd_sel[IRT_ROT_MEM_ROW_BANKS][IRT_ROT_MEM_COL_BANKS] = {0}, rd_mode[IRT_ROT_MEM_ROW_BANKS] = {0},
                     msb_lsb_sel[IRT_ROT_MEM_ROW_BANKS][IRT_ROT_MEM_COL_BANKS]                     = {0},
                     bg_flag[IRT_ROT_MEM_ROW_BANKS][IRT_ROT_MEM_COL_BANKS][IRT_ROT_MEM_BANK_WIDTH] = {0};
                ...
                // IRT_TRACE("Pixel [%d][%d]: YT %d, YB %d, XL %d, XR %d\n", row, col + pixel, i0[e_irt_crdf_arch],
                // i1[e_irt_crdf_arch], j0[e_irt_crdf_arch], j1[e_irt_crdf_arch]);

                irt_top->IRT_RMRM_block->run(j0[e_irt_crdf_arch],
                                             j1[e_irt_crdf_arch],
                                             i0[e_irt_crdf_arch],
                                             i1[e_irt_crdf_arch],
                                             bank_row,
                ...
                if (pixel == 0) { // 1st pixel in group storing its corner, used in fixed point only
                    xi0[e_irt_crdf_arch] = j0[e_irt_crdf_arch];
                    yi0[e_irt_crdf_arch] = i0[e_irt_crdf_arch];
                    ...
                    if (hl_only != 1) {
                        switch (irt_top->irt_cfg.flow_mode) {
                            ...
                            case e_irt_flow_wCFIFO_fixed_adaptive_wxh:
                                if (((YB_fixed - YT_fixed + 1) > IRT_INT_WIN_H) ||
                                    ((XR_fixed - XL_fixed + 1) > IRT_INT_WIN_W)) { // dont fit interpolation window
                                    switch (irt_top->irt_desc[desc].rate_mode) {
                                        case e_irt_rate_adaptive_wxh:
                                            adj_proc_size = pixel; // set for number of already processed pixels in
                                                                   // group
                                            rate_reach_flag = 1;
                                            // IRT_TRACE("HL model %s: for output range[%d][%d:%d] processing rate is
                                            // limited to %d pixels/cycle\n", irt_irt_mode_s[transform_type], row, col,
                                            // col + pixel, adj_proc_size);
                                            break;
                                        case e_irt_rate_fixed: // fixed, error if fixed does not feet window
                                            IRT_TRACE("HL model %s: output range[%d][%d:%d] does not fit input "
                                                      "interpolation window 8x9 for %d processing rate, require %dx%d "
                                                      "window from input range[%d:%d][%d:%d]\n",
                                            ...
                                            IRT_TRACE_TO_RES(
                                                test_res,
                                                " failed, HL model %s: output range[%d][%d:%d] does not fit input "
                                                "interpolation window 8x9 for %d processing rate, require %dx%d window "
                                                "from input range[%d:%d][%d:%d]\n",
                                                irt_irt_mode_s[transform_type],
                                                row,
                                                col,
                                                col + pixel,
                                        ...
                                        case e_irt_rate_adaptive_2x2:
                                            IRT_TRACE("HL model %s: %s rate mode is not supported in %s flow\n",
                                                      irt_irt_mode_s[transform_type],
                                                      irt_rate_mode_s[irt_top->irt_desc[desc].rate_mode],
                                                      irt_flow_mode_s[irt_top->irt_cfg.flow_mode]);
                                            IRT_TRACE_TO_RES(
                                                test_res,
                                                " failed, HL model %s: %s rate mode is not supported in %s flow\n",
                                                irt_irt_mode_s[transform_type],
                                                irt_rate_mode_s[irt_top->irt_desc[desc].rate_mode],
                                                irt_flow_mode_s[irt_top->irt_cfg.flow_mode]);
                                            IRT_CLOSE_FAILED_TEST(0);
                            ...
                            case e_irt_flow_wCFIFO_fixed_adaptive_2x2:
                                switch (irt_top->irt_desc[desc].rate_mode) {
                                    ...
                                    case e_irt_rate_adaptive_2x2:
                                        for (uint8_t row_bank = 0; row_bank < IRT_ROT_MEM_ROW_BANKS; row_bank++) {
                                            for (uint8_t col_bank = 0; col_bank < IRT_ROT_MEM_COL_BANKS; col_bank++) {
                                                if (rot_mem_rd_stat.bank_num_read[row_bank][col_bank] >
                                                        1 /*bank is accessed more than once, can be conflict*/
                                                    &&
                                                    rot_mem_rd_stat.bank_max_addr[row_bank][col_bank] !=
                                                        rot_mem_rd_stat
                                                            .bank_min_addr[row_bank]
                                                                          [col_bank] && // there is conflict on the bank
                                                                                        // because more than 1 different
                                                                                        // addresses are accessed
                                                    rot_mem_rd_stat.bank_max_addr[row_bank][col_bank] != -1 &&
                                                    rot_mem_rd_stat.bank_min_addr[row_bank][col_bank] != 10000) {
                                                    // adj_proc_size = pixel;//set for number of already processed
                                                    // pixels in group
                                                    rate_reach_flag = 1;
#if 0
                        ...
                        if (rate_reach_flag) {
                            ...
                            if (irt_top->irt_desc[desc].rate_mode == e_irt_rate_fixed &&
                                adj_proc_size < irt_top->irt_desc[desc].proc_size) {
                                IRT_TRACE("HL model %s: output range[%d][%d:%d] does not fit %d processing rate, "
                                          "require %dx%d window from input range[%d:%d][%d:%d], maximum rate is %d\n",
                                          irt_irt_mode_s[transform_type],
                                          row,
                                          col,
                                          col + pixel,
                                ...
                                IRT_TRACE_TO_RES(
                                    test_res,
                                    " failed, HL model %s: output range[%d][%d:%d] does not fit %d processing rate, "
                                    "require %dx%d window from input range[%d:%d][%d:%d], maximum rate is %d\n",
                                    irt_irt_mode_s[transform_type],
                                    row,
                                    col,
                            ...
                            adj_proc_size =
                                (uint8_t)std::min((int)adj_proc_size, (int)irt_top->irt_desc[desc].proc_size);
#if 0
                    ...
                    if (pixel >= adj_proc_size - 1) {
                        // min_proc_rate = std::min(adj_proc_size, min_proc_rate); //if (adj_proc_size < min_proc_rate)
                        // min_proc_rate = adj_proc_size; max_proc_rate = std::max(adj_proc_size, max_proc_rate); //if
                        // (adj_proc_size > max_proc_rate) max_proc_rate = adj_proc_size;
                        rate_reach_flag = 0;
                    }
                    if (adj_proc_size < 1 || adj_proc_size > IRT_ROT_MAX_PROC_SIZE) {
                        IRT_TRACE("HL model %s: output range[%d][%d] adj_proc_size %d is error\n",
                                  irt_irt_mode_s[transform_type],
                                  row,
                                  col,
                                  adj_proc_size);
                        IRT_TRACE_TO_RES(test_res,
                                         " failed, HL model %s: output range[%d][%d] adj_proc_size %d is error\n",
                                         irt_irt_mode_s[transform_type],
                                         row,
                                         col,
                                         adj_proc_size);
                ...
                // i1[e_irt_crdf_arch], j0[e_irt_crdf_arch], j1[e_irt_crdf_arch]); IRT_TRACE_DBG(" [%d, %d] [%d, %d]\n",
                // oob_flag_v[e_irt_crdf_arch], oob_flag_h[e_irt_crdf_arch], noint_flag_v[e_irt_crdf_arch],
                // noint_flag_h[e_irt_crdf_arch]);

                // for (int format = 0; format <= e_irt_crdf_fix16; format++)
                // IRT_TRACE_DBG("Before vflip: format %d, [%u, %u]: [%d, %d] [%d, %d], oob_flag_v %d, noint_flag_v %d,
                // oob_flag_h %d, noint_flag_h %d\n", 	format, row, col + pixel, i0[format], i1[format], j0[format],
                // j1[format], oob_flag_v[format], noint_flag_v[format], oob_flag_h[format], noint_flag_h[format]);
                ...
                // i0[format], i1[format], j0[format], j1[format]); IRT_TRACE_DBG("After sat: [%u, %u]: [%d, %d] [%d,
                // %d]\n", row, col + pixel, i0[e_irt_crdf_arch], i1[e_irt_crdf_arch], j0[e_irt_crdf_arch],
                // j1[e_irt_crdf_arch]);
                ...
                for (plane = 0; plane < PLANES; plane++) {
                    ...
                    for (int format = 0; format <= e_irt_crdf_fix16; format++) {
                        ...
                        // assign pixel
                        p_value[0] =
                            (p_valid[0] ? input_image[plane][i0[format]][j0[format]] : 0) & irt_top->irt_desc[desc].Msi;
                        p_value[1] =
                            (p_valid[1] ? input_image[plane][i0[format]][j1[format]] : 0) & irt_top->irt_desc[desc].Msi;
                        p_value[2] =
                            (p_valid[2] ? input_image[plane][i1[format]][j0[format]] : 0) & irt_top->irt_desc[desc].Msi;
                        p_value[3] =
                            (p_valid[3] ? input_image[plane][i1[format]][j1[format]] : 0) & irt_top->irt_desc[desc].Msi;
                        ...
                        i_valid[0] = i0[format] >= 0 && i1[format] < Hi && j0[format] >= 0 && j1[format] < Wi &&
                                     irt_top->irt_desc[desc].read_hflip == 0 && irt_top->irt_desc[desc].read_vflip == 0;
                        i_valid[1] = i0[format] >= 0 && i1[format] < Hi && j1[format] >= 0 && j0[format] < Wi &&
                                     irt_top->irt_desc[desc].read_hflip == 1 && irt_top->irt_desc[desc].read_vflip == 0;
                        i_valid[2] = i1[format] >= 0 && i0[format] < Hi && j0[format] >= 0 && j1[format] < Wi &&
                                     irt_top->irt_desc[desc].read_hflip == 0 && irt_top->irt_desc[desc].read_vflip == 1;
                        i_valid[3] = i1[format] >= 0 && i0[format] < Hi && j1[format] >= 0 && j0[format] < Wi &&
                                     irt_top->irt_desc[desc].read_hflip == 1 && irt_top->irt_desc[desc].read_vflip == 1;
                        ...
                        if (i_valid[0] || i_valid[1] || i_valid[2] || i_valid[3] ||
                            p_valid[0] && irt_top->irt_desc[desc].rot90 && rot90_int == 0) {
                            switch (format) {
                                case e_irt_crdf_arch:
                                    out_fpt32 = p_fpt32[0] * (1 - xf_arch) * (1 - yf_arch) +
                                                p_fpt32[1] * xf_arch * (1 - yf_arch) +
                                                p_fpt32[2] * (1 - xf_arch) * yf_arch + p_fpt32[3] * xf_arch * yf_arch;

                                    xw0       = (float)((double)xf_fix31 / pow(2.0, 31));
                                    xw1       = (float)((pow(2.0, 31) - (double)xf_fix31) / pow(2.0, 31));
                                    yw0       = (float)((double)yf_fix31 / pow(2.0, 31));
                                    yw1       = (float)((pow(2.0, 31) - (double)yf_fix31) / pow(2.0, 31));
                                    out_fpt32 = p_fpt32[0] * xw1 * yw1 + p_fpt32[1] * xw0 * yw1 +
                                                p_fpt32[2] * xw1 * yw0 + p_fpt32[3] * xw0 * yw0;
                                    // horizontal followed by vertical
                                    out_fpt32 = (p_fpt32[0] * xw1 + p_fpt32[1] * xw0) * yw1 +
                                                (p_fpt32[2] * xw1 + p_fpt32[3] * xw0) * yw0;
                                    // vertical vertical by horizontal
                                    out_fpt32 = (p_fpt32[0] * yw1 + p_fpt32[2] * yw0) * xw1 +
                                                (p_fpt32[1] * yw1 + p_fpt32[3] * yw0) * xw0;

                                    out1[format] = (int)rint(out_fpt32 * pow(2, irt_top->irt_desc[desc].bli_shift));

                                    // IRT_TRACE_DBG(" [%f,%f,%f,%f]\n", p_fpt32[0], p_fpt32[1], p_fpt32[2],
                                    // p_fpt32[3]); out1[format] = (int)floor(out_fpt32 * pow(2.0, 9) + 0.5);
                                    // out1[format] = out1[format] >> (8 - (irt_top->rot_pars[desc].Pwo -
                                    // irt_top->rot_pars[desc].Pwi - irt_top->rot_pars[desc].Ppi)); out1[format] =
                                    // (out1[format] + 1) >> 1; IRT_TRACE("BLI: Task %d [%d, %d]:
                                    // pixels[0x%x][0x%x][0x%x][0x%x] w[%f][%f][%f][%f]\n", plane, row, col + pixel,
                                    //	p_value[0], p_value[1], p_value[2], p_value[3], xw0, xw1, yw0, yw1);

                                    break;
                                case e_irt_crdf_fpt64:
                                    // horizontal followed by vertical
                                    out_fpt64 = p_fpt64[0] * (1 - xf_fpt64) * (1 - yf_fpt64) +
                                                p_fpt64[1] * xf_fpt64 * (1 - yf_fpt64) +
                                                p_fpt64[2] * (1 - xf_fpt64) * yf_fpt64 +
                                                p_fpt64[3] * xf_fpt64 * yf_fpt64;

                                    // vertical vertical by horizontal
                                    out_fpt64 = (p_fpt64[0] * (1 - yf_fpt64) + p_fpt64[2] * yf_fpt64) * (1 - xf_fpt64) +
                                                (p_fpt64[1] * (1 - yf_fpt64) + p_fpt64[3] * yf_fpt64) * xf_fpt64;

                                    out1[format] = (int)floor(
                                        out_fpt64 *
                                            pow(2.0, irt_top->rot_pars[desc].Pwo - irt_top->rot_pars[desc].Pwi) +
                                        0.5);
                                    out1[format] = (int)rint(out_fpt64 * pow(2, irt_top->irt_desc[desc].bli_shift));
                                    break;
                                case e_irt_crdf_fpt32:
                                    // horizontal followed by vertical
                                    out_fpt32 = p_fpt32[0] * (1 - xf_fpt32) * (1 - yf_fpt32) +
                                                p_fpt32[1] * xf_fpt32 * (1 - yf_fpt32) +
                                                p_fpt32[2] * (1 - xf_fpt32) * yf_fpt32 +
                                                p_fpt32[3] * xf_fpt32 * yf_fpt32;
                                    // vertical vertical by horizontal
                                ...
                                case e_irt_crdf_fixed:
                                    ...
                                    w[0] = (((int64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) *
                                           (((int64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed);
                                    ...
                                    out_fixed =
                                        p_fixed[0] * w[0] + p_fixed[1] * w[1] + p_fixed[2] * w[2] + p_fixed[3] * w[3];
                                    out1[e_irt_crdf_fixed] = (int)(((out_fixed >> irt_top->rot_pars[desc].TOTAL_PREC) +
                                                                    irt_top->rot_pars[desc].TOTAL_ROUND) >>
                                                                   irt_top->rot_pars[desc].TOTAL_PREC);
                                    out1[e_irt_crdf_fixed] =
                                        (int)(((out_fixed >>
                                                (2 * irt_top->rot_pars[desc].TOTAL_PREC -
                                                 (irt_top->rot_pars[desc].Pwo - irt_top->rot_pars[desc].Pwi) - 1)) +
                                               1) >>
                                              1);
                                    // printf("[%lld, %lld, %lld, %lld]\n",
                                    //	(irt_2_pow_calc_prec - (uint64_t)xf_fixed)*(irt_2_pow_calc_prec -
                                    //(uint64_t)yf_fixed), ((uint64_t)xf_fixed)* (irt_2_pow_calc_prec -
                                    //(uint64_t)yf_fixed), 	(irt_2_pow_calc_prec - (uint64_t)xf_fixed)*
                                    //((uint64_t)yf_fixed), ((uint64_t)xf_fixed)* ((uint64_t)yf_fixed)); printf("[%lld,
                                    // %lld, %lld, %lld]\n", w0 << IRT_COORD_CAL_PREC, w1 << IRT_COORD_CAL_PREC, w2 <<
                                    // IRT_COORD_CAL_PREC, w3 << IRT_COORD_CAL_PREC); printf("[%lld, ", out_fixed >>
                                    // IRT_COORD_CAL_PREC);

                                    // calculating weights with 2*IRT_COORD_CAL_PREC - IRT_WEIGHT_PREC precision to save
                                    // bits after that
                                    w[0] = ((((uint64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) *
                                            (((uint64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed)) >>
                                           irt_top->rot_pars[desc].WEIGHT_PREC;
                                    w[1] =
                                        (xf_fixed * (((uint64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed)) >>
                                        irt_top->rot_pars[desc].WEIGHT_PREC;
                                    w[2] =
                                        ((((uint64_t)1 << irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) * yf_fixed) >>
                                        irt_top->rot_pars[desc].WEIGHT_PREC;
                                    w[3] = (xf_fixed * yf_fixed) >> irt_top->rot_pars[desc].WEIGHT_PREC;
                                    out_fixed =
                                        p_fixed[0] * w[0] + p_fixed[1] * w[1] + p_fixed[2] * w[2] + p_fixed[3] * w[3];
                                    out1[format] = (int)(((out_fixed >> (irt_top->rot_pars[desc].TOTAL_PREC -
                                                                         irt_top->rot_pars[desc].WEIGHT_PREC)) +
                                                          irt_top->rot_pars[desc].TOTAL_ROUND) >>
                                                         irt_top->rot_pars[desc].TOTAL_PREC);
                                    out1[format] = (int)(((out_fixed >> (irt_top->rot_pars[desc].bli_shift_fix -
                                                                         irt_top->rot_pars[desc].WEIGHT_PREC)) +
                                                          1) >>
                                                         1);
                                    // round to half nearest even, e.g. 2.5 -> 2, 3.5 -> 4. We check 2LSB after shift.
                                    // b0.0 -> b0; 0.1 -> 0. ; 1.0 -> 1. ; 1.1 -> 1. +  1;
                                    out_fixed_intp1 = out_fixed >> (irt_top->rot_pars[desc].bli_shift_fix -
                                                                    irt_top->rot_pars[desc].WEIGHT_PREC);
                                    out_fixed_other =
                                        out_fixed - (out_fixed_intp1 << (irt_top->rot_pars[desc].bli_shift_fix -
                                                                         irt_top->rot_pars[desc].WEIGHT_PREC));
                                ...
                                case e_irt_crdf_fix16:
                                    ...
                                    out_fix16 = p_fix16[0] * w16[0] + p_fix16[1] * w16[1] + p_fix16[2] * w16[2] +
                                                p_fix16[3] * w16[3];
                                    out1[format] = (int)(((out_fix16 >> 16) + (1 << 15)) >> 16);
                                    out1[format] =
                                        (int)(((out_fix16 >> (2 * 16 - irt_top->irt_desc[desc].bli_shift - 1)) + 1) >>
                                              1);
                                    out_fixed_intp1 = out_fix16 >> (2 * 16 - irt_top->irt_desc[desc].bli_shift - 1);
                                    out_fixed_other = out_fix16 - (out_fixed_intp1
                                                                   << (2 * 16 - irt_top->irt_desc[desc].bli_shift - 1));
                                    if (out_fixed_other == 0) { // value is .5 or 0.0
                                        switch (out_fixed_intp1 & 0x3) {
                                            case 0: out1[format] = (int)(out_fixed_intp1 >> 1); break; // 0.0 , truncate
                                            case 1:
                            ...
                            if (irt_top->irt_desc[desc].bg_mode == e_irt_bg_prog_value)
                                ...
                                int16_t i0_sat = IRT_top::IRT_UTILS::irt_min_int16(
                                    IRT_top::IRT_UTILS::irt_max_int16(0, i0[format]), Hi - 1);
                                int16_t j0_sat = IRT_top::IRT_UTILS::irt_min_int16(
                                    IRT_top::IRT_UTILS::irt_max_int16(0, j0[format]), Wi - 1);
                                out1[format] = (int)(input_image[plane][i0_sat][j0_sat] & irt_top->irt_desc[desc].Msi);
                                out1[format] =
                                    (int)rint((float)out1[format] * pow(2, irt_top->irt_desc[desc].bli_shift));
                            }
                        }
                        ...
                        out1[format] =
                            std::min(out1[format],
                                     (int)irt_top->irt_desc[desc].MAX_VALo); // if (out1_fpt32 > 255) out1_fpt32 = 255;
                        output_image[image * CALC_FORMATS_ROT + format][plane][row][col + pixel] =
                            (uint16_t)(out1[format] << irt_top->irt_desc[desc].Ppo);
                    }
                }
            }
...
#endif
            min_proc_rate = (uint8_t)std::min(
                (int)adj_proc_size,
                (int)min_proc_rate); // if (adj_proc_size < min_proc_rate) min_proc_rate = adj_proc_size;
            max_proc_rate = (uint8_t)std::max(
                (int)adj_proc_size,
                (int)max_proc_rate); // if (adj_proc_size > max_proc_rate) max_proc_rate = adj_proc_size;
    ...
    IRT_TRACE("HL model %s processing rate statistics\n", irt_irt_mode_s[transform_type]);
        ...
        switch (transform_type) {
            case e_irt_rotation:
                for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fix16; format_type++) {
                    n = sprintf(out_file_name, "%s_%d_rot_%s.bmp", file_name, image, irt_hl_format_s[format_type]);
                    WriteBMP(irt_top,
                             image,
                             PLANES,
                             out_file_name,
                             So,
                             Ho,
                             output_image[image * CALC_FORMATS_ROT + format_type]);
                }
                break;
            case e_irt_affine:
                n = sprintf(out_file_name,
                            "%s_%d_aff_%s_angle_%3.2f_Sx_%3.2f_Sy_%3.2f.bmp",
                            file_name,
                            image,
                ...
                for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fixed; format_type++) {
                    n = sprintf(out_file_name, "%s_%d_aff_%s.bmp", file_name, image, irt_hl_format_s[format_type]);
                    WriteBMP(irt_top,
                             image,
                             PLANES,
                             out_file_name,
            ...
            case e_irt_projection:
                n = sprintf(out_file_name,
                            "%s_%d_prj_%s_roll_%3.2f_pitch_%3.2f_yaw_%3.2f_Sx_%3.2f_Sy_%3.2f_Zd_%3.2f.bmp",
                            file_name,
                            image,
                ...
                //		WriteBMP(image, out_file_name, Wo, Ho);
                for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fixed; format_type++) {
                    n = sprintf(out_file_name, "%s_%d_prj_%s.bmp", file_name, image, irt_hl_format_s[format_type]);
                    WriteBMP(irt_top,
                             image,
                             PLANES,
                             out_file_name,
            ...
            case e_irt_mesh:
                for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fixed; format_type++) {
                    n = sprintf(out_file_name, "%s_%d_mesh_%s.bmp", file_name, image, irt_hl_format_s[format_type]);
                    WriteBMP(irt_top,
                             image,
                             PLANES,
                             out_file_name,
...
void IRThl_fpt32(IRT_top* irt_top,
                 uint8_t  image,
                 uint8_t  planes,
                 char     file_name[50],
                 double   D,
        ...
        for (uint16_t col = 0; col < Wo; col++) {
            ...
            for (uint8_t plane = 0; plane < 3; plane++) {
                ...
                if (input_hflip == 0 && xi0 >= 0 && xi0 + 1 < Wi && input_vflip == 0 && yi0 >= 0 && yi0 + 1 < Hi ||
                    input_hflip == 1 && xoff - xi0 >= 0 && xoff - xi0 + 1 < Wi && input_vflip == 0 && yi0 >= 0 &&
                        yi0 + 1 < Hi ||
                    input_hflip == 0 && xi0 >= 0 && xi0 + 1 < Wi && input_vflip == 1 && yoff - yi0 >= 0 &&
                        yoff - yi0 + 1 < Hi ||
                    input_hflip == 1 && xoff - xi0 >= 0 && xoff - xi0 + 1 < Wi && input_vflip == 1 && yoff - yi0 >= 0 &&
                        yoff - yi0 + 1 < Hi) {
                    ...
                    if (plane == 0)
                        fprintf(hl_out_file,
                                "[yo][xo]=[%d][%d]: [yi0][xi0]=[%d][%d], [yf][xf]=[%f][%f], out %f\n",
                                row,
                                col,
                                input_vflip ? yoff - yi0 : yi0,
                                input_hflip ? xoff - xi0 : xi0,
                                input_vflip ? 1 - yf : yf,
                                input_hflip ? 1 - xf : xf,
                                out_val);
                } else
...
void IRThl_fixed(IRT_top* irt_top,
                 uint8_t  image,
                 uint8_t  planes,
                 char     file_name[50],
                 double   D,
                 ...
                 int      suffix)
{
    ...
    double cosDr = (float)(floor(cosD * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                           pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    double sinDr = (float)(floor(sinD * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                           pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    ...
    double M11Dr =
        (float)(floor(irt_top->rot_pars[desc].M11d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    double M21Dr =
        (float)(floor(irt_top->rot_pars[desc].M21d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    double M12Dr =
        (float)(floor(irt_top->rot_pars[desc].M12d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    double M22Dr =
        (float)(floor(irt_top->rot_pars[desc].M22d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    ...
    printf("\nRunning fixed point high level model on image %d %s %dx%d with %3.2f degree rotation with [h/v]flip "
           "[%d/%d]\n",
           image,
           file_name,
           Wi,
    ...
    FILE* hl_out_file;
    n           = sprintf(out_file_name, "%s_%d_rot_fixed%s.txt", file_name, image, suffix == 0 ? "" : "_flip");
    hl_out_file = fopen(out_file_name, "w");
    ...
    for (uint8_t plane = 0; plane < 3; plane++)
        for (uint16_t row = 0; row < Ho; row++)
            for (uint16_t col = 0; col < Wo; col += proc_size) {
                ...
                if (remain_pixels > proc_size)
                    remain_pixels = proc_size;
                XL = 1000;
                YT = 1000, YB = -1000, XR = -1000;
                for (int i = 0; i < remain_pixels; i++) {
                    ...
                    double cosDi_fpt32 = floor(i * cosD * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                                         pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC);
                    double sinDi_fpt32 = floor(i * sinD * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1) /
                                         pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC);

                    int M11i_int = (int)floor(
                        i * irt_top->rot_pars[desc].M11d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1);
                    int M21i_int = (int)floor(
                        i * irt_top->rot_pars[desc].M21d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) + 0.5 * 1);
                    M11i_int = i * M11_int;
                    M21i_int = i * M21_int;
                    double M11i_fpt32 =
                        floor(i * irt_top->rot_pars[desc].M11d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) +
                              0.5 * 1) /
                        pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC);
                    double M21i_fpt32 =
                        floor(i * irt_top->rot_pars[desc].M21d * pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) +
                              0.5 * 1) /
                        pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC);
                    ...
                    if (irt_top->irt_desc[desc].irt_mode == e_irt_rotation) {
                        ...
                        xi_fixed = (int64_t)cosD_int * xo + (int64_t)sinD_int * yo +
                                   Xic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
                        yi_fixed = -(int64_t)sinD_int * xo + (int64_t)cosD_int * yo +
                                   Yic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));

                        xi_fixed = (int64_t)cosD_int * ((int64_t)xo - i) + (int64_t)sinD_int * yo + cosDi_int +
                                   Xic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
                        yi_fixed = -(int64_t)sinD_int * ((int64_t)xo - i) + (int64_t)cosD_int * yo - sinDi_int +
                                   Yic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));

                    } else {

                        xi_fixed = (int64_t)M11_int * ((int64_t)xo - i) + (int64_t)M12_int * yo + M11i_int +
                                   Xic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
                        yi_fixed = (int64_t)M21_int * ((int64_t)xo - i) + (int64_t)M22_int * yo + M21i_int +
                                   Yic * ((uint64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
                    }
                    ...
                    if (abs(xi0 - xi0_fixed) > 1) {
                        printf("Error: at pixel[%d,%d] xi0 != xi0_fixed (%d != %ld) at image %d\n",
                               row,
                               col,
                               xi0,
                               xi0_fixed,
                    ...
                    if (abs(yi0 - yi0_fixed) > 1) {
                        printf("Error: at pixel[%d,%d] yi0 != yi0_fixed (%d != %ld) at image %d\n",
                               row,
                               col + i,
                               yi0,
                               yi0_fixed,
                    ...
                    if (fabs(xi_fpt32) >= 2048)
                        thr *= 2;
                    if (fabs(xi_fpt32) >= 1024)
                        thr *= 2;
                    if (fabs(xi_fpt32) >= 512)
                        thr *= 2;
                    if (fabs(xi_fpt32 - ((float)xi_fixed) / (float)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC)) >
                        thr) {
                        printf("Error: at pixel[%d,%d] xi_fpt32 != xi_fixed (%f != %f) at image %d\n",
                               row,
                               col + i,
                               xi_fpt32,
                               xi_fixed / pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC),
                               image);
                        exit(0);
                    }
                    thr = (float)(4 * pow(2.0, -irt_top->rot_pars[desc].TOTAL_PREC));
                    if (fabs(yi_fpt32) >= 2048)
                        thr *= 2;
                    if (fabs(yi_fpt32) >= 1024)
                        thr *= 4;
                    if (fabs(yi_fpt32) >= 512)
                        thr *= 2;
                    if (fabs(yi_fpt32 - ((float)yi_fixed) / (float)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC)) >
                        thr) {
                        printf("Error: at pixel[%d,%d] yi_fpt32 != yi_fixed (%f != %f) at image %d\n",
                               row,
                               col + i,
                               yi_fpt32,
                               yi_fixed / pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC),
                               image);
                        exit(0);
                    }

                    if (fabs(xf_fpt32 - ((float)xf_fixed) / (float)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC)) >
                            2 * (float)pow(2.0, -irt_top->rot_pars[desc].TOTAL_PREC) &&
                        warning_print[0] == 0) {
                        printf("Warning: at pixel[%4d,%4d] xf_fpt32 != xf_fixed (%f != %f) at image %d\n",
                               row,
                               col + i,
                               xf_fpt32,
                               xf_fixed / pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC),
                               image);
                        warning_print[0] = 1;
                        // exit(0);
                    }
                    if (fabs(yf_fpt32 - ((float)yf_fixed) / (float)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC)) >
                            2 * (float)pow(2.0, -irt_top->rot_pars[desc].TOTAL_PREC) &&
                        warning_print[1] == 0) {
                        printf("Warning: at pixel[%4d,%4d] yf_fpt32 != yf_fixed (%f != %f) at image %d\n",
                               row,
                               col + i,
                               yf_fpt32,
                               yf_fixed / pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC),
                               image);
                        warning_print[1] = 1;
                        // exit(0);
                    }
                    ...
                    if (i == 0) {
                        fprintf(
                            hl_out_file, "Plane %d, pixel[%3d,%3d], [yo,xo]=[%3d,%3d]\n", plane, row, col, yo, xo - i);
                        fprintf(hl_out_file,
                                ...
                                yi_fixed,
                                remain_pixels - 1,
                                (int64_t)cosD_int * ((int64_t)xo + remain_pixels - 1) + (int64_t)sinD_int * yo +
                                    Xic * (int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC),
                                -(int64_t)sinD_int * ((int64_t)xo + remain_pixels - 1) + (int64_t)cosD_int * yo +
                                    Yic * (int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
                    ...
                    fprintf(
                        hl_out_file,
                        "%d: [y00=%2d,x00=%2d], [%2x,%2x,%2x,%2x] [%4llx,%4llx] [%d,%d,%d,%d]\n",
                        i,
                        yi0_fixed,
                        xi0_fixed,
                        (uint32_t)((uint8_t)p0),
                        ...
                        (yi0_fixed + 1) < 0 || (yi0_fixed + 1) > (Hi - 1) || (xi0_fixed + 1) < 0 ||
                            (xi0_fixed + 1) > (Wi - 1));
                    ...
                    if ((xi0_fixed >= 0 && xi0_fixed + 1 < Wi && yi0_fixed >= 0 && yi0_fixed + 1 < Hi) &&
                            (input_hflip == 0) && (input_vflip == 0) ||
                        (xi0_fixed >= 1 && xi0_fixed < Wi && yi0_fixed >= 0 && yi0_fixed + 1 < Hi) &&
                            (input_hflip == 1) && (input_vflip == 0) ||
                        (xi0_fixed >= 0 && xi0_fixed + 1 < Wi && yi0_fixed >= 1 && yi0_fixed < Hi) &&
                            (input_hflip == 0) && (input_vflip == 1) ||
                        (xi0_fixed >= 1 && xi0_fixed < Wi && yi0_fixed >= 1 && yi0_fixed < Hi) && (input_hflip == 1) &&
                            (input_vflip == 1)) {

                        // cout << height << " " << width << " " << plane << " " << (int)
                        // (input_image[plane][yi_int][xi_int]) << endl;
                        if (input_hflip == 0 && input_vflip == 0) {
                            ...
                            out_val_fpt32 = (uint32_t)((uint8_t)input_image[plane][yi0_fixed][xi0_fixed]) *
                                                (pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) *
                                                (pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed) +
                                            (uint32_t)((uint8_t)input_image[plane][yi0_fixed][xi0_fixed + 1]) *
                                                xf_fixed * (pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed) +
                                            (uint32_t)((uint8_t)input_image[plane][yi0_fixed + 1][xi0_fixed]) *
                                                (pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) * yf_fixed +
                                            (uint32_t)((uint8_t)input_image[plane][yi0_fixed + 1][xi0_fixed + 1]) *
                                                ((double)xf_fixed) * ((double)yf_fixed);

                            out_val_fixed =
                                (uint64_t)((uint8_t)input_image[plane][yi0_fixed][xi0_fixed]) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed) +
                                (uint64_t)((uint8_t)input_image[plane][yi0_fixed][xi0_fixed + 1]) * (xf_fixed) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed) +
                                (uint64_t)((uint8_t)input_image[plane][yi0_fixed + 1][xi0_fixed]) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) * (yf_fixed) +
                                (uint64_t)((uint8_t)input_image[plane][yi0_fixed + 1][xi0_fixed + 1]) * (xf_fixed) *
                                    (yf_fixed);

                        } else {
                            out_val_fpt32 =
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0][Wi - 1 - xi0]) * (xf_fpt32) *
                                    (yf_fpt32) +
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0][Wi - 1 - xi0 + 1]) *
                                    (1 - xf_fpt32) * (yf_fpt32) +
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0 + 1][Wi - 1 - xi0]) * (xf_fpt32) *
                                    (1 - yf_fpt32) +
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0 + 1][Wi - 1 - xi0 + 1]) *
                                    (1 - xf_fpt32) * (1 - yf_fpt32);

                            out_val_fpt32 =
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed][Wi - 1 - xi0_fixed]) *
                                    ((double)xf_fixed) * ((double)yf_fixed) +
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed][Wi - 1 - xi0_fixed + 1]) *
                                    ((double)(pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed)) * yf_fixed +
                                (uint32_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed + 1][Wi - 1 - xi0_fixed]) *
                                    xf_fixed * ((pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed)) +
                                (uint32_t)(
                                    (uint8_t)input_image[plane][Hi - 1 - yi0_fixed + 1][Wi - 1 - xi0_fixed + 1]) *
                                    ((pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed)) *
                                    ((pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed));

                            out_val_fixed =
                                (uint64_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed][Wi - 1 - xi0_fixed]) *
                                    (xf_fixed) * (yf_fixed) +
                                (uint64_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed][Wi - 1 - xi0_fixed + 1]) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) * (yf_fixed) +
                                (uint64_t)((uint8_t)input_image[plane][Hi - 1 - yi0_fixed + 1][Wi - 1 - xi0_fixed]) *
                                    (xf_fixed) * ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed) +
                                (uint64_t)(
                                    (uint8_t)input_image[plane][Hi - 1 - yi0_fixed + 1][Wi - 1 - xi0_fixed + 1]) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - xf_fixed) *
                                    ((int64_t)pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC) - yf_fixed);
                        ...
                        out_val_fixed =
                            (int)floor((floor((double)out_val_fixed / pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC)) +
                                        pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC - 1)) /
                                       pow(2.0, irt_top->rot_pars[desc].TOTAL_PREC));
    ...
    n = sprintf(out_file_name, "%s_%d_rot_fixed%s.bmp", file_name, image, suffix == 0 ? "" : "_flip");
...
// Resampler HL Model
//----------------------------------------------------------------------------------------
void update_grad_data(int c, int x, int y, float value, uint16_t Hi, uint16_t Wi)
{
    if (x >= 0 && y >= 0 && x <= Wi - 1 && y <= Hi - 1) {
        IRT_TRACE_TO_LOG(4,
                         hl_out_file_tmp,
                         "update_grad_data[%d][%d][%d] :: %f + %f = ",
                         c,
                         x,
                         y,
                         input_im_grad_data[c][y][x],
                         value);
        input_im_grad_data[c][y][x] += value;
        IRT_TRACE_TO_LOG(
            4,
            hl_out_file_tmp,
            "%f : %x \n",
            input_im_grad_data[c][y][x],
            IRT_top::IRT_UTILS::conversion_float_bit(input_im_grad_data[c][y][x], e_irt_fp32, 0, 0, 0, 0, 0, 0, 0));
    }
}

// TODO - FRAME EXTENTION MODE
// //TODO - Need to add support for bg_mode 0 BG pixel and bg_mode 2 IP padding
float get_data_point(int c, int x, int y, uint16_t Hi, uint16_t Wi, int bg_mode = 1)
{
    float out;
    int   sat_x = min(max(0, x), Wi - 1);
    int   sat_y = min(max(0, y), Hi - 1);

    if ((x < 0 || x > (Wi - 1) || y < 0 || y > (Hi - 1)) && (bg_mode != 1))
        return 0;
    else
        return out = input_im_data[c][sat_y][sat_x];
}

void IRThl_resampler(IRT_top* irt_top, uint8_t image, uint8_t planes)
{
    ...
    uint16_t               Hm        = irt_top->irt_desc[desc].image_par[MIMAGE].H;
    uint16_t               Wm        = irt_top->irt_desc[desc].image_par[MIMAGE].W;
    Eirt_resamp_dtype_enum out_dtype = irt_top->irt_desc[desc].image_par[OIMAGE].DataType;
    Eirt_resamp_dtype_enum in_dtype  = (irt_top->irt_desc[desc].irt_mode == 7)
                                          ? irt_top->irt_desc[desc].image_par[MIMAGE].DataType
                                          : irt_top->irt_desc[desc].image_par[IIMAGE].DataType;
    int16_t  bli_shift = irt_top->irt_desc[desc].bli_shift;
    uint16_t MAX_VALo  = irt_top->irt_desc[desc].MAX_VALo;
    uint16_t Ppo       = irt_top->irt_desc[desc].Ppo;
    uint8_t  irt_mode  = (int)irt_top->irt_desc[desc].irt_mode;
    uint16_t out_ps    = (irt_mode != 5) ? (1 << irt_top->irt_desc[desc].image_par[OIMAGE].Ps)
                                      : (1 << (irt_top->irt_desc[desc].image_par[OIMAGE].Ps - 1));
    // float nudge = irt_top->rot_pars[desc].nudge;
    // rescale arguments
    // TODO  -- Gh/Gv should be converted back from descriptor
    // float Gh =
    // IRT_top::IRT_UTILS::conversion_bit_float(irt_top->irt_desc[desc].mesh_Gh,irt_top->irt_desc[desc].image_par[OIMAGE].DataType);
    // float Gv =
    // IRT_top::IRT_UTILS::conversion_bit_float(irt_top->irt_desc[desc].mesh_Gv,irt_top->irt_desc[desc].image_par[OIMAGE].DataType);
    float Gh = (irt_mode == 7) ? irt_top->irt_desc[desc].rescale_LUT_x->Gf : 0;
    float Gv = (irt_mode == 7) ? irt_top->irt_desc[desc].rescale_LUT_y->Gf : 0;
    //---------------------------------------
    uint8_t       Htaps    = (irt_mode == 7) ? irt_top->irt_desc[desc].rescale_LUT_x->num_taps : 0;
    uint8_t       Vtaps    = (irt_mode == 7) ? irt_top->irt_desc[desc].rescale_LUT_y->num_taps : 0;
    int           Hoff     = (Htaps % 2 == 0) ? (Htaps / 2) - 1 : (Htaps / 2);
    int           Voff     = (Vtaps % 2 == 0) ? (Vtaps / 2) - 1 : (Vtaps / 2);
    filter_type_t flt_type = irt_top->rot_pars[desc].filter_type;

    uint16_t bg_mode            = irt_top->irt_desc[desc].bg_mode;
    uint16_t resize_bli_grad_en = irt_top->irt_desc[desc].resize_bli_grad_en;
    uint16_t relative_mode_en   = irt_top->irt_desc[desc].mesh_rel_mode;
    uint16_t int_mode           = irt_top->irt_desc[desc].int_mode;

    float    gx   = irt_top->irt_desc[desc].M11f; // TODO replace with descr - M11 affine coeff also convrt to float
    float    gy   = irt_top->irt_desc[desc].M22f; // TODO replace wit descr - M22 affine coeff also convrt to float
    float    Xc_o = ((float)irt_top->irt_desc[desc].image_par[OIMAGE].Xc) / 2;
    float    Yc_o = ((float)irt_top->irt_desc[desc].image_par[OIMAGE].Yc) / 2;
    float    Xc_i = ((float)irt_top->irt_desc[desc].image_par[IIMAGE].Xc) / 2;
    float    Yc_i = ((float)irt_top->irt_desc[desc].image_par[IIMAGE].Yc) / 2;

    char out_file_name[50];

    FILE* hl_out_file;
    sprintf(out_file_name, "IRThl_resampler_Image%d_rot.txt", image);
    hl_out_file     = fopen(out_file_name, "w");
    hl_out_file_tmp = hl_out_file;

    IRT_TRACE_TO_LOG(4,
                     hl_out_file,
                     "Running IRThl_resampler at %s mode irt_mode=%d\n",
                     irt_irt_mode_s[irt_top->irt_desc[desc].irt_mode],
                     irt_mode);
    IRT_TRACE_TO_LOG(4,
                     hl_out_file,
                     ...
                     irt_irt_mode_s[irt_top->irt_desc[desc].irt_mode],
                     irt_mode);
    // IRT_TRACE_TO_LOG(4,hl_out_file,"out_ps=%d\n",out_ps);
    switch (irt_mode) {
        case 4:
            IRT_TRACE_TO_LOG(
                4, hl_out_file, "output_image:: [image][ch][line][pix] :: InPix :: OutputFloat :: OutputHex :: Grad\n");
            break;
        case 5:
            IRT_TRACE_TO_LOG(
                4, hl_out_file, "output_image:: [image][ch][line][pix] :: InPix :: OutputFloat :: OutputHex :: Grad\n");
        ...
        case 6:
            IRT_TRACE_TO_LOG(
                4,
                hl_out_file,
                "output_image:: [image][ch][line][pix] :: GradOut :: GradData [Float] :: GradData [Hex]\n");
            break;
        case 7:
            IRT_TRACE_TO_LOG(4,
                             hl_out_file,
                             "Taps[H:V] = [%d : %d] Phase = %d\n",
                             Htaps,
                             Vtaps,
                             irt_top->irt_desc[desc].rescale_LUT_x->num_phases);
            break;
    }

    float p_sat_x = (bg_mode == 2) ? (float)Wi : (float)Wi - 1;
    float p_sat_y = (bg_mode == 2) ? (float)Hi : (float)Hi - 1;
    float n_sat_x = (bg_mode == 2) ? (float)-1 : (float)0;
    float n_sat_y = (bg_mode == 2) ? (float)-1 : (float)0;

    // - assertion to be added
    // TODO -
    // . resize_grad to be enabled in BWD2 only or resize-Grad should be ignored in all other modes
    // . relative mode == 0 -> when resize_grad == 1 & vise versa
    //------------------------------
    IRT_TRACE_TO_LOG(4, hl_out_file, "Hi:Wi %d:%d Ho:Wo %d:%d Hm:Wm %d:%d\n", Hi, Wi, Ho, Wo, Hm, Wm);
    for (int ch_id = 0; ch_id < planes; ch_id++) {
        uint8_t bw2_en  = (irt_top->irt_desc[desc].irt_mode == e_irt_resamp_bwd2) ? 1 : 0;
        // uint8_t wstride = (bw2_en == 1) ? (irt_top->irt_desc[desc].warp_stride << 1) : 0;
        uint8_t wstride = (bw2_en == 1) ? ((irt_top->irt_desc[desc].warp_stride == 1)
                                               ? 2
                                               : ((irt_top->irt_desc[desc].warp_stride == 3) ? 4 : 0))
                                        : 0;
        for (int oimg_h = 0; oimg_h < Ho; oimg_h++) {
            //---------------------------------------
            // shuffle logic
            if (wstride > 0) {
                uint16_t idx = 0;
                ...
                for (int k = 0; k < wstride; k++) {
                    uint16_t num_elm = (int)floor(Wo / wstride) + (((Wo % wstride) > k) ? 1 : 0);
                    for (int l = 0; l < num_elm; l++) {
                        wl[idx][1] = m_warp_data[oimg_h][k + (l * wstride)][1];
                        wl[idx][0] = m_warp_data[oimg_h][k + (l * wstride)][0];
                        gl[idx]    = output_im_grad_data[ch_id][oimg_h][k + (l * wstride)];
                        idx++;
                    }
                }
                //---------------------------------------
                ...
                for (int l = 0; l < Wo; l++) {
                    m_warp_data[oimg_h][l][1]             = wl[l][1];
                    m_warp_data[oimg_h][l][0]             = wl[l][0];
                    output_im_grad_data[ch_id][oimg_h][l] = gl[l];
                }
            }
            //---------------------------------------
            for (int oimg_w = 0; oimg_w < Wo; oimg_w++) {
                float Tx = (resize_bli_grad_en & bw2_en) ? m_warp_data[oimg_h][oimg_w][1]
                                                         : (relative_mode_en ? (m_warp_data[oimg_h][oimg_w][1] + oimg_w)
                                                                             : m_warp_data[oimg_h][oimg_w][1]);
                float Ty = (resize_bli_grad_en & bw2_en) ? m_warp_data[oimg_h][oimg_w][0]
                                                         : (relative_mode_en ? (m_warp_data[oimg_h][oimg_w][0] + oimg_h)
                                                                             : m_warp_data[oimg_h][oimg_w][0]);
                float Wx         = (Tx < n_sat_x) ? n_sat_x : (Tx > p_sat_x) ? p_sat_x : Tx;
                float Wy         = (Ty < n_sat_y) ? n_sat_y : (Ty > p_sat_y) ? p_sat_y : Ty;
                int   fx         = (int)floor(Wx);
                int   fy         = (int)floor(Wy);
                int   cx         = (int)ceil(Wx); // fx+1;
                int   cy         = (int)ceil(Wy); // fy+1;
                float xf         = Wx - (float)fx;
                float yf         = Wy - (float)fy;
                ...
                float im_fxfy_P0 = get_data_point(ch_id, fx, fy, Hi, Wi);
                float im_cxfy_P1 = get_data_point(ch_id, cx, fy, Hi, Wi);
                float im_fxcy_P2 = get_data_point(ch_id, fx, cy, Hi, Wi);
                float im_cxcy_P3 = get_data_point(ch_id, cx, cy, Hi, Wi);
                float grad_out   = output_im_grad_data[ch_id][oimg_h][oimg_w];
                float xi, yi, im_P;
                int   imPidx[4], hpi, vpi; // only for print
                float tmpr1;
                //---------------------------------------
                if (irt_top->irt_desc[desc].irt_mode == e_irt_resamp_fwd) {
                    // FWD Pass
                    // IRT_TRACE_TO_LOG(4,hl_out_file,"before output_im_data\n");
                    output_im_data[ch_id][oimg_h][oimg_w] = ((1 - xf) * (((1 - yf) * im_fxfy_P0) + (yf * im_fxcy_P2))) +
                                                            (xf * (((1 - yf) * im_cxfy_P1) + (yf * im_cxcy_P3)));
                    // IRT_TRACE_TO_LOG(4,hl_out_file,"before m_warp_grad_data\n");
                }
                //---------------------------------------
                if (irt_top->irt_desc[desc].irt_mode == e_irt_resamp_bwd1) {
                    // BWD pass1
                    m_warp_grad_data[oimg_h][oimg_w][1] +=
                        grad_out * ((1 - yf) * (im_cxfy_P1 - im_fxfy_P0) + (yf) * (im_cxcy_P3 - im_fxcy_P2));
                    m_warp_grad_data[oimg_h][oimg_w][0] +=
                        grad_out * ((1 - xf) * (im_fxcy_P2 - im_fxfy_P0) + (xf) * (im_cxcy_P3 - im_cxfy_P1));
                    IRT_TRACE_TO_LOG(4,
                                     hl_out_file,
                                     " h[%d]w[%d] grad_out[%f] p3:p0[%f:%f:%f:%f] (1-yf)[%f] p1-p0[%f] yf[%f] "
                                     "p3-p2[%f] grad_data[[1][%f]\n",
                                     oimg_h,
                                     oimg_w,
                                     grad_out,
                                     im_cxcy_P3,
                                     ...
                                     im_fxfy_P0,
                                     1 - yf,
                                     im_cxfy_P1 - im_fxfy_P0,
                                     yf,
                                     im_cxcy_P3 - im_fxcy_P2,
                                     m_warp_grad_data[oimg_h][oimg_w][1]);
                    IRT_TRACE_TO_LOG(4,
                                     hl_out_file,
                                     " h[%d]w[%d] grad_out[%f] p3:p0[%f:%f:%f:%f]  (1-xf)[%f] p2-p0[%f] xf[%f] "
                                     "p3-p1[%f] grad_data[[1][%f]\n",
                                     oimg_h,
                                     oimg_w,
                                     grad_out,
                                     im_cxcy_P3,
                                     ...
                                     im_fxfy_P0,
                                     1 - xf,
                                     im_fxcy_P2 - im_fxfy_P0,
                                     xf,
                                     im_cxcy_P3 - im_cxfy_P1,
                                     m_warp_grad_data[oimg_h][oimg_w][0]);
                }
                //---------------------------------------
                if (irt_top->irt_desc[desc].irt_mode == e_irt_resamp_bwd2) {
                    // BWD pass2
                    // IRT_TRACE_TO_LOG(4,hl_out_file,"update_grad_data \n");
                    update_grad_data(ch_id, fx, fy, grad_out * ((1 - xf) * (1 - yf)), Hi, Wi);
                    update_grad_data(ch_id, cx, fy, grad_out * ((xf) * (1 - yf)), Hi, Wi);
                    update_grad_data(ch_id, fx, cy, grad_out * ((1 - xf) * (yf)), Hi, Wi);
                    update_grad_data(ch_id, cx, cy, grad_out * ((xf) * (yf)), Hi, Wi);
                }
                //---------------------------------------
                if (irt_top->irt_desc[desc].irt_mode == e_irt_rescale) {
                    // rescale
                    IRT_TRACE_TO_LOG(4,
                                     hl_out_file,
                                     "=============================\n",
                                     ...
                                     vpi,
                                     hpi);
                    xi = (Gh * (((float)oimg_w) - ((float)irt_top->irt_desc[desc].image_par[OIMAGE].Xc / 2))) +
                         ((float)irt_top->irt_desc[desc].image_par[IIMAGE].Xc / 2); // TODO NUDGE
                    yi = (Gv * (((float)oimg_h) - ((float)irt_top->irt_desc[desc].image_par[OIMAGE].Yc / 2))) +
                         ((float)irt_top->irt_desc[desc].image_par[IIMAGE].Yc / 2);
                    xf  = xi - (int)floor(xi);
                    yf  = yi - (int)floor(yi);
                    hpi = (int)rint(xf *
                                    (float)irt_top->irt_desc[desc]
                                        .rescale_LUT_x->num_phases); // phase index //TODO - RINT for
                                                                     // round-to-nearest -- Done. Check with Arch team
                    vpi = (int)rint(yf * (float)irt_top->irt_desc[desc].rescale_LUT_y->num_phases);
                    // Horizontal interpolation
                    IRT_TRACE_TO_LOG(4,
                                     hl_out_file,
                                     "Yo:Xo [%d][%d] xi:yi[%f][%f] xf:yf %f:%f Htaps:Vtaps %d:%d vpi:hpi %d:%d\n",
                                     oimg_h,
                                     oimg_w,
                                     xi,
                                     yi,
                                     ...
                                     vpi,
                                     hpi);
                    for (int hi = 0; hi < Htaps; hi++) {
                        int Xflr = (int)floor(xi);
                        int Yflr = (int)floor(yi);
                        int hti  = (Xflr - Hoff + hi); // tap index
                        // vertic interpolation
                        im_P = 0;
                        for (int vi = 0; vi < Vtaps; vi++) {
                            int   vti  = (Yflr - Voff + vi); // tap index
                            float tmpr = get_data_point(ch_id, hti, vti, Hi, Wi, 1);
                            if (vi + 1 <= Vtaps) {
                                tmpr1 = get_data_point(ch_id, hti, vti + 1, Hi, Wi, 1);
                            }
                            if (in_dtype == e_irt_int8 || in_dtype == e_irt_int16) {
                                int tempP = ((uint16_t)tmpr) & irt_top->irt_desc[desc].Msi;
                                tmpr      = (float)tempP;
                                if (vi + 1 <= Vtaps) {
                                    int tempP1 = ((uint16_t)tmpr1) & irt_top->irt_desc[desc].Msi;
                                    tmpr1      = (float)tempP1;
                                }
                            }
                            // im_P += get_rescale_coeff(flt_type,irt_top->irt_desc[desc].rescale_LUT_y,vpi,vti) *
                            // get_data_point(ch_id,hti,vti,Hi,Wi);
                            if (vi + 1 <= Vtaps) {
                                im_P += IRT_top::IRT_UTILS::get_rescale_coeff(
                                            flt_type, irt_top->irt_desc[desc].rescale_LUT_y, vpi, vi) *
                                            tmpr +
                                        IRT_top::IRT_UTILS::get_rescale_coeff(
                                            flt_type, irt_top->irt_desc[desc].rescale_LUT_y, vpi, vi + 1) *
                                            tmpr1; // TODO pass bg mode from desc
                            } else {
                                im_P += IRT_top::IRT_UTILS::get_rescale_coeff(
                                            flt_type, irt_top->irt_desc[desc].rescale_LUT_y, vpi, vi) *
                                        tmpr; // TODO pass bg mode from desc
                            }
                            //---------------------------------------
                            IRT_TRACE_TO_LOG(
                                4,
                                hl_out_file,
                                "VERTICAL ACCUM : vti:hti=%d:%d coeff [%f][%x] InPix [%f][%x] OutAccum [%f][%x]\n",
                                vti,
                                hti,
                                IRT_top::IRT_UTILS::get_rescale_coeff(
                                    flt_type, irt_top->irt_desc[desc].rescale_LUT_y, vpi, vi),
                                IRT_top::IRT_UTILS::conversion_float_bit(
                                    ...
                                    0,
                                    0),
                                get_data_point(ch_id, hti, vti, Hi, Wi, 1),
                                IRT_top::IRT_UTILS::conversion_float_bit(
                                    get_data_point(ch_id, hti, vti, Hi, Wi, 1), in_dtype, 0, 0, 0, 0, 0, 0, 0),
                                im_P,
                                IRT_top::IRT_UTILS::conversion_float_bit(im_P, e_irt_fp32, 0, 0, 0, 0, 0, 0, 0));
                            if (vi == 0 && hi == 0) {
                                imPidx[0] = vti;
                                imPidx[0] = hti;
                            }
                            if (vi == Vtaps - 1 && hi == Htaps - 1) {
                                imPidx[0] = vti;
                                imPidx[0] = hti;
                            }
                            if (vi + 1 <= Vtaps) {
                        ...
                        output_im_data[ch_id][oimg_h][oimg_w] +=
                            IRT_top::IRT_UTILS::get_rescale_coeff(
                                flt_type, irt_top->irt_desc[desc].rescale_LUT_x, hpi, hi) *
                            im_P;
                        IRT_TRACE_TO_LOG(4,
                                         hl_out_file,
                                         "==>output_pix[%d][%d] = OutAccum [%f] * coeff [%f][%x] = [%f][%x] \n",
                                         oimg_h,
                                         oimg_w,
                                         im_P,
                                         IRT_top::IRT_UTILS::get_rescale_coeff(
                                         ...
                                         output_im_data[ch_id][oimg_h][oimg_w],
                                         IRT_top::IRT_UTILS::conversion_float_bit(
                                             output_im_data[ch_id][oimg_h][oimg_w], e_irt_fp32, 0, 0, 0, 0, 0, 0, 0));
                        // IRT_TRACE_TO_LOG(4,hl_out_file,"output_im[%d][%d] = %f im_P %f coord %f
                        // \n",oimg_h,oimg_w,output_im_data[ch_id][oimg_h][oimg_w],im_P,IRT_top::IRT_UTILS::get_rescale_coeff(flt_type,irt_top->irt_desc[desc].rescale_LUT_x,hpi,hti));
                    }
                }
                //---------------------------------------
                // update out_image for FWD for now
                switch (irt_mode) {
                    case 4:
                        output_image[image][ch_id][oimg_h][oimg_w] = IRT_top::IRT_UTILS::conversion_float_bit(
                            output_im_data[ch_id][oimg_h][oimg_w], out_dtype, bli_shift, MAX_VALo, Ppo, 0, 0, 0, 0);
                        IRT_TRACE_TO_LOG(4,
                                         hl_out_file,
                                         " [%d][%d][%d][%d] :: [%f][%f][%f][%f] :: [%f] :: [%x]\n",
                                         image,
                                         ...
                                         im_cxcy_P3,
                                         output_im_data[ch_id][oimg_h][oimg_w],
                                         output_image[image][ch_id][oimg_h][oimg_w]);
                        break;
                    case 5: // TODO - output_image is allocated fro 8k x 8k x 3 PLanes for 2 byte mode.. I would creash
                            // for bwd-1,2 modes for large images.. need to allocade larger arrays
                        output_image[image][ch_id][oimg_h][oimg_w] = IRT_top::IRT_UTILS::conversion_float_bit(
                            m_warp_grad_data[oimg_h][oimg_w][1], out_dtype, bli_shift, MAX_VALo, Ppo, 0, 0, 0, 0);
                        // uint32_t tmp = IRT_top::IRT_UTILS::conversion_float_bit(m_warp_grad_data[oimg_h][oimg_w][0],
                        // out_dtype, bli_shift, MAX_VALo, Ppo); output_image[image][ch_id][oimg_h][oimg_w] =
                        // output_image[image][ch_id][oimg_h][oimg_w] | (((uint64_t)tmp)<<(8*out_ps));
                        output_image[image][ch_id][oimg_h][oimg_w] =
                            output_image[image][ch_id][oimg_h][oimg_w] |
                            (((uint64_t)(IRT_top::IRT_UTILS::conversion_float_bit(
                                 m_warp_grad_data[oimg_h][oimg_w][0], out_dtype, bli_shift, MAX_VALo, Ppo, 0, 0, 0, 0)))
                             << (8 * out_ps));
                        IRT_TRACE_TO_LOG(
                            4,
                            hl_out_file,
                            " [%d][%d][%d][%d] :: [%f][%f][%f][%f] :: [%f][%f] :: [%lx] :: [%lx][%lx] :: [%f] \n",
                            image,
                            ch_id,
                            oimg_h,
                            oimg_w,
                ...
                // memcpy(&output_image[image][ch_id][oimg_h][oimg_w], &output_im_data[ch_id][oimg_h][oimg_w],
                // sizeof(float));
            }
        }
    }
    if (irt_mode == 6) {
        for (int ch_id = 0; ch_id < planes; ch_id++) {
            for (int iimg_h = 0; iimg_h < Hi; iimg_h++) {
                for (int iimg_w = 0; iimg_w < Wi; iimg_w++) {
                    float grad_out                             = output_im_grad_data[ch_id][iimg_h][iimg_w];
                    output_image[image][ch_id][iimg_h][iimg_w] = IRT_top::IRT_UTILS::conversion_float_bit(
                        input_im_grad_data[ch_id][iimg_h][iimg_w], out_dtype, bli_shift, MAX_VALo, Ppo, 0, 0, 0, 0);
                    IRT_TRACE_TO_LOG(4,
                                     hl_out_file,
                                     " [%d][%d][%d][%d] :: [%f] :: [%f] :: [%x]\n",
                                     image,
                                     ...
                                     iimg_w,
                                     grad_out,
                                     input_im_grad_data[ch_id][iimg_h][iimg_w],
                                     output_image[image][ch_id][iimg_h][iimg_w]);
                }
            }
        }
    }
    IRT_TRACE_TO_LOG(4, hl_out_file, " Done Successfully\n");
    fclose(hl_out_file);
    IRT_TRACE(" HL Done Successfully\n");
}
//----------------------------------------------------------------------------------------
// End of Resampler HL Model
//----------------------------------------------------------------------------------------
void IRThl(IRT_top* irt_top, uint8_t image, uint8_t planes, uint16_t i_width, uint16_t i_height, char* file_name)
{
    ...
    // image, file_name, i_width, i_height);
    IRThl_advanced_transform(irt_top, image, planes, file_name, irt_top->irt_desc[desc].irt_mode);

    if (print_out_files) {
        for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fix16; format_type++) {
            n = sprintf(out_file_name, "%s_%d_%s.bmp", file_name, image, irt_hl_format_s[format_type]);
            WriteBMP(irt_top,
                     image,
                     PLANES,
                     out_file_name,
        ...
        for (uint8_t format_type = e_irt_crdf_arch; format_type <= e_irt_crdf_fix16; format_type++) {
            n = sprintf(out_file_name, "IRT_image_%d_out_%s.bmp", image, irt_hl_format_s[format_type]);
            WriteBMP(irt_top,
                     image,
                     PLANES,
                     out_file_name,
